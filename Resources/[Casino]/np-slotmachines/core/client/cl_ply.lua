...
local function drawStatusHUD(state, pValues)
  ...
  local values = {}
  isSlotMachinesEnabled, currentJackPot = RPC.execute("np-slotmachines-sv:checkIfSlotMachinesState")
  ...
  -- exports["np-textui"]:sendAppEvent("status-hud", {
  --   show = state,
  --   title = title,
  --   values = values
  -- })
    ...
    if isSeatedAtSlotMachine then
      if not isBetContextShowing then
        if currentSlotMachineDetails ~= nil then
          ...
          table.insert(data, {
            action = "np-textui-slotmachines:placeBet",
            title = "Place bet",
            description = "$100.00",
            key = {
              coinValue = 100,
          ...
          table.insert(data, {
            action = "np-textui-slotmachines:leaveMachine",
            title = "Leave Machine",
            description = "",
            key = {},
          })
        ...
        while true do
          ...
          if not isNear(plyPos, v.chairPos, 0.75) then
            if isInteractionShowing then
              ...
              exports["np-textui"]:hideInteraction(interactionBackground)
            end
          ...
          if not isInteractionShowing then
            ...
            exports["np-textui"]:showInteraction("[E] Play " .. v.label, interactionBackground)
          end
...
--#[NUI Callbacks]#--
RegisterUICallback("np-textui-slotmachines:placeBet", function(data, cb)
...
RegisterUICallback("np-textui-slotmachines:leaveMachine", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = "done" } })
  getUpFromSlotMachine()
  SendNUIMessage({
    app = "status-hud",
