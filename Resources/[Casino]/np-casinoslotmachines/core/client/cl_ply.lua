...
--     --   values = values
--     -- })
--     exports["np-textdisplay"]:DisplayText({
--       header = title,
--         rows = {
--           {
--               id = 0, 
...
--   })
    ...
    exports["np-textdisplay"]:HideText()
  end
end
        ...
        while true do
          ...
          if not isNear(plyPos, v.chairPos, 0.75) then
            if isInteractionShowing then
              ...
              exports["np-interaction"]:hideInteraction(interactionBackground)
            end
          ...
          if not isInteractionShowing then
            ...
            if exports["np-inventory"]:hasEnoughOfItem("casinomembership", 1, false, true) then
            ...
            exports["np-interaction"]:showInteraction("[E] Play " .. v.label, interactionBackground)
          end
...
AddEventHandler("np-slotmachines-cl:spinFinished", function(amountWon)
  if amountWon > 0 then
    ...
    TriggerEvent("player:receiveItem", "casinochips", amountWon)
...
RegisterUICallback("np-ui-slotmachines:placeBet", function(data, cb)
  ...
  if currentSlotMachine and not isSlotMachineSpinning then
    ...
    if not exports["np-inventory"]:hasEnoughOfItem("casinochips", currentBet, false, true) then 
      TriggerEvent("DoLongHudText", "You do not have enough in your casino balance to bet.", 2)
      return
    else
      TriggerEvent("inventory:removeItem", "casinochips", currentBet)
    end
    --Wait(2000)
    ...
    local title = "Slot Machine - " .. currentSlotMachineLabel
    --local balance = RPC.execute("np-casino:getCurrentChipCount")
    local balance = exports["np-inventory"]:getQuantity("casinochips")
    --local values = {}
    --table.insert(values, "Balance: $" .. balance)
      exports["np-textdisplay"]:DisplayText({
      header = title,
...
function spinSlot(tickRate)
  ...
  for i=1,300,1 do
    ...
    elseif i == 180 then
      ...
      TriggerEvent("np-slotmachines-cl:stopReels", 1, ang1)
    end
    if i < 240 then
    elseif i == 240 then
      ang2 = tickRate.b*22.5-180+0.0
      print(ang2)
      TriggerEvent("np-slotmachines-cl:stopReels", 2, ang2)
    end
    if i < 300 then
    elseif i == 300 then
      ang3 = tickRate.c*22.5-180+0.0
      TriggerEvent("np-slotmachines-cl:stopReels", 3, ang3)
    end				
    Citizen.Wait(10)
  end
  TriggerServerEvent('np-slotmachines-sv:slotsCheckWin', tickRate, data)
end

RegisterNetEvent('np-slotmachines-cl:startSpin')
AddEventHandler('np-slotmachines-cl:startSpin',function(tickRate)
  spinSlot(tickRate)
end)
