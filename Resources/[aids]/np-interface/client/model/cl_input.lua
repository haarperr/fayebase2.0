...
RegisterNUICallback('np-interface:applicationClosed', function(data, cb)
    if (data.name ~= 'textbox' or data.callbackUrl ~= 'np-interface:inputResponse') then return end
...
RegisterNUICallback('np-interface:inputResponse', function(data, cb)
...
function OpenInputMenu(pEntries, pValidation)
    ...
    exports['np-interface']:openApplication('textbox', {
        callbackUrl = 'np-interface:inputResponse',
        key = inputId,
        items = pEntries,
        show = true,
    })
