...
RegisterNetEvent('np-civjobs:postop:cancel')
AddEventHandler('np-civjobs:postop:cancel', function(groupId, members)
...
function isAtDropOff()
    ...
    Citizen.CreateThread(function()
        while isNearDropOff do
            ...
            if dist <= 2.0 then
                canPlyDrop = 1
                exports['np-interface']:showInteraction('[F1] Drop Off Package')
            else
                canPlyDrop = 0
...
RegisterNetEvent('np-jobs:drop_package')
AddEventHandler('np-jobs:drop_package', function()
    exports['np-interface']:hideInteraction()
    TaskStartScenarioInPlace(PlayerPedId(), "PROP_HUMAN_BUM_BIN", 0, true)
    local finished = exports['np-taskbar']:taskBar(2500, 'Dropping Goods')
