...
AddEventHandler('BeginDivingjob', function()
    local timer = 5000
    local finished = exports["np-taskbar"]:taskBar(timer,"Checking your cash")
    if finished then
...
AddEventHandler('getDiveingjob', function()
  TriggerEvent('np-jobs:SpawnDivingBoat')
  Wait(2000)
  print('HERE')
  StartDive()
end)
...
function StartDive()
    ...
    if active['active'] == 1 then
        ...
        addBlip(rand)
        if run == 0 then
            TriggerEvent("addEmail", {title = "Boss", subject = "Job Offer", message = "Yo, So I've got another location for you. It should be marked on your GPS... I think its a " .. tostring(active['info'])})
        elseif run == 1 then
            TriggerEvent("addEmail", {title = "Boss", subject = "Job Offer", message = "Yo, So I've got another location for you. It should be marked on your GPS... I think its a " .. tostring(active['info'])})
        elseif run == 2 then
            TriggerEvent("addEmail", {title = "Boss", subject = "Job Offer", message = "Yo, So I've got another location for you. It should be marked on your GPS... I think its a " .. tostring(active['info'])})
                ...
                if dis <= 20 and value['active'] == 1 then
                    ...
                    if dis <= 5 then
                        if IsControlJustReleased(0, 38) then
                            ...
                            local finished = exports["np-taskbar"]:taskBar(waiter,"Searching For Treasure")
                            if finished == 100 then
                            value['active'] = 0
                            scavanged = scavanged + 1
                            FreezeEntityPosition(PlayerPedId(), false)
                            ClearPedTasksImmediately(PlayerPedId())
                            TriggerServerEvent('np-scuba:findTreasure')
...
function SellItems()
    if exports["np-inventory"]:hasEnoughOfItem("ironbar",10,false) then 
        ...
        TriggerServerEvent('np-scuba:paySalvage', 250)
    else
        TriggerEvent('DoLongHudText', 'You dont have enough Material in your pockets to sell!', 2)
    end
end

RegisterNetEvent('np-jobs:SpawnDivingBoat')
AddEventHandler('np-jobs:SpawnDivingBoat', function()
	Citizen.Wait(1)
