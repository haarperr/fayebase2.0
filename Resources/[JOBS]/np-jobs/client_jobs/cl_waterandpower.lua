...
function voidrpCoolHaha(ElectricalLocation,ElectricalPoint)
...
RegisterNetEvent('np-civjobs:waternpower:cancel')
AddEventHandler('np-civjobs:waternpower:cancel', function(groupId, members)
...
AddEventHandler("waterAndPowerIsAtLocation", function(groupId, members)
    ...
    for k,v in pairs(members) do
        if tonumber(v.src) == tonumber(src) then
            ...
            voidrpCoolHaha(ElectricalLocation,ElectricalPoint)
            isNearBox = true
            isAtBox()
        end
    end
...
function isAtElectricalBox()
    ...
    Citizen.CreateThread(function()
        while isNearElectricalBox do
            ...
            if dist <= 2.0 then
                canPlyFix = 1
                exports['np-interface']:showInteraction('[F1] Fix Electricals')
            else
                canPlyFix = 0
...
RegisterNetEvent('np-jobs:fix_elec')
AddEventHandler('np-jobs:fix_elec', function()
    FreezeEntityPosition(GetPlayerPed(-1), true)
    TriggerEvent("animation:PlayAnimation","welding")
    canPlyFix = 0
    isNearBox = false
    exports['np-interface']:hideInteraction()
    local canwork = exports['np-taskbar']:taskBar(15000, 'Fixing Electricals')
    if (canwork == 100) then
