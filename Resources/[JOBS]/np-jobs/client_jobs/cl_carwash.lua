			...
			for i = 1, #vehicleWashStation do
				...
				if #(GetEntityCoords(PlayerPedId()) - vector3(garageCoords2[1], garageCoords2[2], garageCoords2[3])) < 5 then
					if isCleaning == false then
						exports['np-interface']:showInteraction("Press [ENTER] to clean your vehicle!")
					else
						exports['np-interface']:hideInteraction()
					end
					if(IsControlJustPressed(1, Key)) and isCleaning == false then
						TriggerServerEvent('np-carwash:checkmoney')
					end
				end
			end
...
RegisterNetEvent('np-carwash:success')
AddEventHandler('np-carwash:success', function(price)
	TriggerEvent('DoLongHudText', 'Vehicle Clean $' .. price ..'!')
	FreezeEntityPosition(GetVehiclePedIsUsing(PlayerPedId()), true)
	isCleaning = true
	local finished = exports["np-taskbar"]:taskBar(10000,"Cleaning Vehicle",false,false,playerVeh)
...
RegisterNetEvent('np-carwash:notenoughmoney')
AddEventHandler('np-carwash:notenoughmoney', function(moneyleft)
