...
Citizen.CreateThread(function()
    exports["np-polyzone"]:AddBoxZone("cosmic_picking", vector3(164.48, -238.71, 50.06), 4.6, 4.8,  {
		name="cosmic_picking",
        heading=340,
        minZ=48.26,
        maxZ=52.26
    ...
    exports["np-polyzone"]:AddBoxZone("cosmic_roof_lower", vector3(196.9, -268.35, 50.02), 5, 5.4, {
    ...
    exports["np-polyzone"]:AddBoxZone("cosmic_roof_upper", vector3(184.98, -263.91, 63.15), 5, 7.0, {
    ...
    exports["np-polyzone"]:AddBoxZone("cosmic_inside_upper", vector3(185.6, -236.04, 54.07), 1, 1.4, {
    ...
    exports["np-polyzone"]:AddBoxZone("cosmic_inside_down", vector3(201.78, -241.1, 65.74), 1, 1.2, {
...
AddEventHandler('np-polyzone:enter', function(name)
    if name == "cosmic_picking" then
        local isEmployed = exports["np-business"]:IsEmployedAt("cosmic_cannabis")
        local HasPermission = exports["np-business"]:HasPermission('cosmic_cannabis',"craft_access")
...
RegisterInterfaceCallback('np-jobs:ls_cf', function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
    if exports['np-inventory']:hasEnoughOfItem('weedq', 1) then
        FreezeEntityPosition(GetPlayerPed(-1),true)
        TriggerEvent("animation:PlayAnimation","cokecut")
        local finished = exports['np-taskbar']:taskBar(math.random(20000, 30000), 'Crafting LS Confidential')
        if (finished == 100) then
...
RegisterInterfaceCallback('np-jobs:af', function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
    if exports['np-inventory']:hasEnoughOfItem('weedq', 1) then
        FreezeEntityPosition(GetPlayerPed(-1),true)
        TriggerEvent("animation:PlayAnimation","cokecut")
        local finished = exports['np-taskbar']:taskBar(math.random(20000, 30000), 'Alaskan Thunder Fuck')
        if (finished == 100) then
            TriggerEvent("inventory:removeItem","weedq", 1)
            Wait(1000)
            FreezeEntityPosition(GetPlayerPed(-1),false)
...
RegisterInterfaceCallback('np-jobs:ck_j', function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
    if exports['np-inventory']:hasEnoughOfItem('weedq', 1) then
        FreezeEntityPosition(GetPlayerPed(-1),true)
        TriggerEvent("animation:PlayAnimation","cokecut")
        local finished = exports['np-taskbar']:taskBar(math.random(20000, 30000), 'Chiliad Kush')
        if (finished == 100) then
            TriggerEvent("inventory:removeItem","weedq", 1)
            Wait(1000)
            FreezeEntityPosition(GetPlayerPed(-1),false)
...
RegisterNetEvent('np-jobs:EdiblesMenu')
AddEventHandler('np-jobs:EdiblesMenu', function()
    local isEmployed = exports["np-business"]:IsEmployedAt("cosmic_cannabis")
    local HasPermission = exports["np-business"]:HasPermission('cosmic_cannabis',"craft_access")
    if isEmployed and HasPermission then
        local CCE = {
        ...
        exports["np-interface"]:showContextMenu(CCE)
    else
        TriggerEvent('DoLongHudText', 'You are not employed at this business', 2)
    end
end)

RegisterInterfaceCallback('np-jobs:brownies', function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
    if exports['np-inventory']:hasEnoughOfItem('weedq', 1) then
        FreezeEntityPosition(GetPlayerPed(-1),true)
        TriggerEvent("animation:PlayAnimation","cokecut")
        local finished = exports['np-taskbar']:taskBar(math.random(10000, 15000), 'Making Cannabis Brownies')
        if (finished == 100) then
            TriggerEvent("inventory:removeItem","weedq", 1)
            Wait(1000)
            FreezeEntityPosition(GetPlayerPed(-1),false)
...
RegisterInterfaceCallback('np-jobs:absinthe', function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
    if exports['np-inventory']:hasEnoughOfItem('weedq', 1) then
        FreezeEntityPosition(GetPlayerPed(-1),true)
        TriggerEvent("animation:PlayAnimation","cokecut")
        local finished = exports['np-taskbar']:taskBar(math.random(10000, 15000), 'Making Cannabis Absinthe')
        if (finished == 100) then
            TriggerEvent("inventory:removeItem","weedq", 1)
            Wait(1000)
            FreezeEntityPosition(GetPlayerPed(-1),false)
...
RegisterInterfaceCallback('np-jobs:gummies', function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
    if exports['np-inventory']:hasEnoughOfItem('weedq', 1) then
        FreezeEntityPosition(GetPlayerPed(-1),true)
        TriggerEvent("animation:PlayAnimation","cokecut")
        local finished = exports['np-taskbar']:taskBar(math.random(10000, 15000), 'Making Cannabis Gummies')
        if (finished == 100) then
            TriggerEvent("inventory:removeItem","weedq", 1)
            Wait(1000)
            FreezeEntityPosition(GetPlayerPed(-1),false)
...
RegisterInterfaceCallback('np-jobs:420', function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
    if exports['np-inventory']:hasEnoughOfItem('weedq', 1) then
        FreezeEntityPosition(GetPlayerPed(-1),true)
        TriggerEvent("animation:PlayAnimation","cokecut")
        local finished = exports['np-taskbar']:taskBar(math.random(10000, 15000), 'Making 420 Bar')
        if (finished == 100) then
            TriggerEvent("inventory:removeItem","weedq", 1)
            Wait(1000)
            FreezeEntityPosition(GetPlayerPed(-1),false)
...
RegisterNetEvent("np-cosmic:tray_1")
AddEventHandler("np-cosmic:tray_1", function()
    TriggerEvent("server-inventory-open", "1", "cosmic-table-1");
end)

RegisterNetEvent("np-cosmic:tray_2")
AddEventHandler("np-cosmic:tray_2", function()
