...
RegisterNetEvent('np-jobs:make_coffee')
AddEventHandler('np-jobs:make_coffee', function()
    local isEmployed = exports["np-business"]:IsEmployedAt("uwu_cafe")
    if isEmployed then
        if exports['np-inventory']:hasEnoughOfItem('coffeebeans', 1) then
            FreezeEntityPosition(GetPlayerPed(-1), true)
            TriggerEvent('animation:PlayAnimation', 'browse')
            local finished = exports['np-taskbar']:taskBar(5000, 'Making Coffee')
...
AddEventHandler('radical-coffee:strawberry:shortcake', function()
    local isEmployed = exports["np-business"]:IsEmployedAt("uwu_cafe")
    if isEmployed then
        if exports['np-inventory']:hasEnoughOfItem('flour', 1) and exports['np-inventory']:hasEnoughOfItem('sugar', 1) and exports['np-inventory']:hasEnoughOfItem('butter', 1) and exports['np-inventory']:hasEnoughOfItem('milk', 1) then
            FreezeEntityPosition(GetPlayerPed(-1),true)
            TriggerEvent("animation:PlayAnimation","cokecut")
            SetEntityHeading(GetPlayerPed(-1), 272.1259765625)
            local finished = exports['np-taskbar']:taskBar(5000, 'Preparing Shortcake')
            if (finished == 100) then
                FreezeEntityPosition(GetPlayerPed(-1),false)
...
RegisterNetEvent('void-radical-coffee:chocolate-cake')
AddEventHandler('void-radical-coffee:chocolate-cake', function()
    local isEmployed = exports["np-business"]:IsEmployedAt("uwu_cafe")
    if isEmployed then
        if exports['np-inventory']:hasEnoughOfItem('butter', 1) and exports['np-inventory']:hasEnoughOfItem('flour', 1) and exports['np-inventory']:hasEnoughOfItem('chocolate_chips', 1) and exports['np-inventory']:hasEnoughOfItem('milk', 1) then
            FreezeEntityPosition(GetPlayerPed(-1),true)
            TriggerEvent("animation:PlayAnimation","cokecut")
            SetEntityHeading(GetPlayerPed(-1), 272.1259765625)
            local finished = exports['np-taskbar']:taskBar(5000, 'Preparing Cake')
            if (finished == 100) then
                FreezeEntityPosition(GetPlayerPed(-1),false)
                TriggerEvent('inventory:removeItem', 'butter', 1)
                TriggerEvent('inventory:removeItem', 'flour', 1)
...
RegisterNetEvent('np-jobs:prepare_donut_radical_coffee')
AddEventHandler('np-jobs:prepare_donut_radical_coffee', function()
    local isEmployed = exports["np-business"]:IsEmployedAt("uwu_cafe")
    if isEmployed then
        if exports['np-inventory']:hasEnoughOfItem('flour', 1) and exports['np-inventory']:hasEnoughOfItem('milk', 1) and exports['np-inventory']:hasEnoughOfItem('sugar', 1) then 
            FreezeEntityPosition(GetPlayerPed(-1),true)
            TriggerEvent("animation:PlayAnimation","cokecut")
            SetEntityHeading(GetPlayerPed(-1), 272.1259765625)
            local finished = exports['np-taskbar']:taskBar(5000, 'Preparing Donut')
            if (finished == 100) then
                TriggerEvent('inventory:removeItem', 'flour', 1)
                TriggerEvent('inventory:removeItem', 'milk', 1)
                TriggerEvent('inventory:removeItem', 'sugar', 1)
...
RegisterNetEvent('np-radical_coffee-make:cola')
AddEventHandler('np-radical_coffee-make:cola', function()
    if exports['np-inventory']:hasEnoughOfIteim('empty_cup', 1) and exports['np-inventory']:hasEnoughOfItem('sugar', 1) and exports['np-inventory']:hasEnoughOfItem('water') then
        local finished = exports['np-taskbar']:taskBar(4000, 'Pouring Cola')
        if (finished == 100) then
...
RegisterNetEvent('void-radical-coffee:fridge')
AddEventHandler('void-radical-coffee:fridge', function()
    local isEmployed = exports["np-business"]:IsEmployedAt("uwu_cafe")
    if isEmployed then
...
RegisterNetEvent('void-radical-coffee:deserts')
AddEventHandler('void-radical-coffee:deserts', function()
    local isEmployed = exports["np-business"]:IsEmployedAt("uwu_cafe")
    if isEmployed then
        local MakeP = {
            {
...
RegisterInterfaceCallback('np-jobs:bb_1', function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
    TriggerEvent('player:receiveItem', 'bento_box_1', 1)
end)

RegisterInterfaceCallback('np-jobs:bb_2', function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
    TriggerEvent('player:receiveItem', 'bento_box_2', 1)
end)

RegisterInterfaceCallback('np-jobs:bb_3', function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
    TriggerEvent('player:receiveItem', 'bento_box_3', 1)
end)

RegisterInterfaceCallback('np-jobs:ss', function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
    TriggerEvent('radical-coffee:strawberry:shortcake')
end)

RegisterInterfaceCallback('np-jobs:cc', function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
    TriggerEvent('void-radical-coffee:chocolate-cake')
end)

RegisterInterfaceCallback('np-jobs:pd', function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
    TriggerEvent('np-jobs:prepare_donut_radical_coffee')
end)

RegisterNetEvent('np-jobs:radical_coffee:coffee_machine')
AddEventHandler('np-jobs:radical_coffee:coffee_machine', function()
    local isEmployed = exports["np-business"]:IsEmployedAt("uwu_cafe")
    if isEmployed then
        local MakeC = {
            {
            ...
            },
            {
                ...
                key = "SS",
                action = 'np-jobs:make_cof',
            },
        }
        exports["np-interface"]:showContextMenu(MakeC)
    else
        TriggerEvent('DoLongHudText', 'Fuck off POLITELY', 2)
    end
end)

RegisterInterfaceCallback('np-jobs:make_cof', function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
    TriggerEvent('np-jobs:make_coffee')
end)

-- Radical Coffee Seats | Table On Right

RegisterNetEvent('np-radical-coffeeSeats_front_1_middle')
AddEventHandler('np-radical-coffeeSeats_front_1_middle', function()
...
RegisterNetEvent('np-radical-coffeeSeats_back_1_middle')
AddEventHandler('np-radical-coffeeSeats_back_1_middle', function()
    SetEntityHeading(GetPlayerPed(-1), 175.74803161621)
...
RegisterNetEvent('np-radical-coffeeSeats_front_2_middle')
AddEventHandler('np-radical-coffeeSeats_front_2_middle', function()
    SetEntityHeading(GetPlayerPed(-1), 0.0)
    SetEntityCoords(PlayerPedId(), -573.80988769531,-1063.6614990234,21.512084960938)
    TriggerEvent('animation:Chair2')
end)

RegisterNetEvent('np-radical-coffeeSeats_back_2_middle')
AddEventHandler('np-radical-coffeeSeats_back_2_middle', function()
    SetEntityHeading(GetPlayerPed(-1), 0.0)
...
RegisterNetEvent('np-radical-coffeeSeats_front_1_left_table')
AddEventHandler('np-radical-coffeeSeats_front_1_left_table', function()
    SetEntityHeading(GetPlayerPed(-1), 0.0)
    SetEntityCoords(PlayerPedId(), -573.87689208984,-1060.0016845703,21.455776367188)
    TriggerEvent('animation:Chair2')
end)

RegisterNetEvent('np-radical-coffeeSeats_front_2_left_table')
AddEventHandler('np-radical-coffeeSeats_front_2_left_table', function()
    SetEntityHeading(GetPlayerPed(-1), 175.74803161621)
...
RegisterNetEvent('np-radical-coffeeSeats_front_3_left_table')
AddEventHandler('np-radical-coffeeSeats_front_3_left_table', function()
    SetEntityHeading(GetPlayerPed(-1), 175.74803161621)
    SetEntityCoords(PlayerPedId(), -572.96704101562,-1059.4549560547,21.495239257812)
    TriggerEvent('animation:Chair2')
end)

RegisterNetEvent('np-radical-coffeeSeats_back_4_left_table')
AddEventHandler('np-radical-coffeeSeats_back_4_left_table', function()
    SetEntityHeading(GetPlayerPed(-1), 0.0)
...
RegisterNetEvent('np-radical-coffeeSeats_front_1_right_table') -- Front Right
AddEventHandler('np-radical-coffeeSeats_front_1_right_table', function()
    SetEntityHeading(GetPlayerPed(-1), 0.0)
    SetEntityCoords(PlayerPedId(), -573.85052490234,-1067.4066162109,21.513159179688)
    TriggerEvent('animation:Chair2')
end)

RegisterNetEvent('np-radical-coffeeSeats_front_3_right_table') -- Front Left
AddEventHandler('np-radical-coffeeSeats_front_3_right_table', function()
    SetEntityHeading(GetPlayerPed(-1), 175.74803161621)
    SetEntityCoords(PlayerPedId(), -573.89013671875,-1066.8395996094,21.525850585938)
    TriggerEvent('animation:Chair2')
end)

RegisterNetEvent('np-radical-coffeeSeats_back_2_right_table') -- Back Left
AddEventHandler('np-radical-coffeeSeats_back_2_right_table', function()
    SetEntityHeading(GetPlayerPed(-1), 175.74803161621)
    SetEntityCoords(PlayerPedId(), -572.96704101562,-1066.8395996094,21.504313476562)
    TriggerEvent('animation:Chair2')
end)

RegisterNetEvent('np-radical-coffeeSeats_back_4_right_table') -- Back Right
AddEventHandler('np-radical-coffeeSeats_back_4_right_table', function()
    SetEntityHeading(GetPlayerPed(-1), 0.0)
    SetEntityCoords(PlayerPedId(), -573.11206054688,-1067.5120849609,21.452622070312)
    TriggerEvent('animation:Chair2')
end)
  
RegisterNetEvent('np-jobs:uwu_toys')
AddEventHandler('np-jobs:uwu_toys', function()
