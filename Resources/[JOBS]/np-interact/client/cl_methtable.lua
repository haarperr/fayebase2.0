...
RegisterNetEvent('np-meth:place_table')
AddEventHandler('np-meth:place_table', function()
    PlacedTable = true
    TriggerEvent('inventory:removeItem', 'methtable', 1)
    TriggerEvent("np-drugs:place-meth", `bkr_prop_meth_table01a`)
end)

RegisterNetEvent("np-drugs:place-meth-bench")
AddEventHandler("np-drugs:place-meth-bench", function(model, coords)
    RequestModel(model)
    CreatedObjects = CreateObject(model, coords)
    FreezeEntityPosition(CreatedObjects, true)
    TriggerServerEvent("np-drugs:new-meth-bench", model, coords)
end)
...
RegisterNetEvent("np-drugs:place-meth")
AddEventHandler("np-drugs:place-meth", function(model)
    exports['np-interface']:showInteraction('[E] Confirm Placement')
    ...
    while voidDrugsMethB do
        ...
        if IsControlJustReleased(0, 46) then
            ...
            local finished = exports['np-taskbar']:taskBar(7000, 'Placing')
            if finished == 100 then
                TriggerEvent("np-drugs:place-meth-bench", model, placement)
...
RegisterNetEvent('np-meth:start_cook')
AddEventHandler('np-meth:start_cook', function()
...
RegisterNetEvent('np-meth:prepare-goods')
AddEventHandler('np-meth:prepare-goods', function()
    if StartedCook and not PreparedGoods then
        FreezeEntityPosition(GetPlayerPed(-1), true)
        TriggerEvent("animation:PlayAnimation","browse")
        local finished = exports['np-taskbar']:taskBar(30000, 'Preparing Goods')
        if finished == 100 then
...
RegisterNetEvent('np-meth:combine-goods')
AddEventHandler('np-meth:combine-goods', function()
    if StartedCook and PreparedGoods and not CombinedGoods then
        FreezeEntityPosition(GetPlayerPed(-1), true)
        TriggerEvent("animation:PlayAnimation","type")
        local finished = exports['np-taskbar']:taskBar(60000,'Combine Goods')
        if finished == 100 then
            FreezeEntityPosition(GetPlayerPed(-1), false)
            CombinedGoods = true
            TriggerEvent('DoLongHudText', 'Step completed.', 1)
...
RegisterNetEvent('np-meth:mix_in_water')
AddEventHandler('np-meth:mix_in_water', function()
    if StartedCook and PreparedGoods and CombinedGoods and not MixedInWater then
        ...
        Wait(50)
        TaskPlayAnim(GetPlayerPed(-1),"weapon@w_sp_jerrycan","fire",2.0, -8, -1,49, 0, 0, 0, 0)
        local finished = exports['np-taskbar']:taskBar(45000,'Mix In Water')
        if finished == 100 then
            FreezeEntityPosition(GetPlayerPed(-1), false)
            MixedInWater = true
            TriggerEvent('DoLongHudText', 'Step completed.', 1)
...
RegisterNetEvent('np-meth:add_solvent')
AddEventHandler('np-meth:add_solvent', function()
    if StartedCook and PreparedGoods and CombinedGoods and MixedInWater and not AddedSolvent then
        ...
        Wait(50)
        TaskPlayAnim(GetPlayerPed(-1),"weapon@w_sp_jerrycan","fire",2.0, -8, -1,49, 0, 0, 0, 0)
        local finished = exports['np-taskbar']:taskBar(50000,'Adding Solvent')
        if finished == 100 then
            FreezeEntityPosition(GetPlayerPed(-1), false)
...
RegisterNetEvent('np-meth:crystalize_product')
AddEventHandler('np-meth:crystalize_product', function()
    if StartedCook and PreparedGoods and CombinedGoods and MixedInWater and AddedSolvent and not Crystalized then
        FreezeEntityPosition(GetPlayerPed(-1), true)
        TriggerEvent("animation:PlayAnimation","browse")
        local finished = exports['np-taskbar']:taskBar(120000,'Crystalizing Product')
        if finished == 100 then
            FreezeEntityPosition(GetPlayerPed(-1), false)
            Crystalized = true
            TriggerEvent('DoLongHudText', 'Step completed.', 1)
...
RegisterNetEvent('np-meth:pack-product')
AddEventHandler('np-meth:pack-product', function()
    if StartedCook and PreparedGoods and CombinedGoods and MixedInWater and AddedSolvent and Crystalized and not Packed then
        FreezeEntityPosition(GetPlayerPed(-1), true)
        TriggerEvent("animation:PlayAnimation","browse")
        local finished = exports['np-taskbar']:taskBar(170000,'Packing Product')
        if finished == 100 then
            FreezeEntityPosition(GetPlayerPed(-1), false)
            Packed = true
            TriggerEvent('DoLongHudText', 'Step completed.', 1)
            TriggerEvent('player:receiveItem', 'methlabcured', 1)
            TriggerEvent('np-meth:cooldown')
        else
            FreezeEntityPosition(GetPlayerPed(-1), false)
        end
    end
...
RegisterNetEvent('np-methtable:pick_up')
AddEventHandler('np-methtable:pick_up', function()
    if PlacedTable then
        TriggerEvent("animation:PlayAnimation","mechanic3")
        FreezeEntityPosition(PlayerPedId(), true)
        local finished = exports['np-taskbar']:taskBar(10000 ,'Picking Up Table')
        if finished == 100 then
...
RegisterNetEvent('np-meth:cooldown')
AddEventHandler('np-meth:cooldown', function()
    CooldownCook = true
    Citizen.Wait(3.6e+6)
    StartedCook = false
    PreparedGoods = false
    ...
    data = {
        {
            ...
            icon = "temperature-high",
            event = "np-meth:crystalize_product",
            parameters = {}
        }
    },
    options = {
