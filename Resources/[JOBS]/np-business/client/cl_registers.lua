...
AddEventHandler('np-business:registerPurchasePrompt', function(pParameters, pEntity, pContext)
    ...
    exports['np-interface']:showContextMenu(acceptContext)
end)
...
AddEventHandler('np-business:registerChargePrompt', function(pParameters, pEntity, pContext)
    exports['np-interface']:openApplication('textbox', {
        callbackUrl = 'np-interface:business:charge',
...
RegisterInterfaceCallback('np-business:finishPurchasePrompt', function (data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
    local success, message = RPC.execute("np-business:completePurchase", data.key)
    if not success then
        TriggerEvent("DoLongHudText", message, 2)
    end
end)

RegisterInterfaceCallback("np-interface:business:charge", function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
