...
RegisterCommand("creationmenu", function()
	...
	local hasPerm = RPC.execute("np-business:hasPermission", "ace_industries", "craft_access")
	
	if employed and hasPerm then
		TriggerEvent("np-business:stash:creationMenu")
	end
end)
...
RegisterInterfaceCallback("np-business:stash:enteredAuthCode", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = "done" } })
  exports["np-interface"]:closeApplication("textbox")
  hasPdAuthCode = true
end)
AddEventHandler("np-interface:application-closed", function(name)
...
function enterPoly(name, data)
  ...
  exports["np-interface"]:showInteraction("[E] " .. name)
end
...
AddEventHandler("np-business:stash:creationMenu", function ()
  ...
  exports["np-interface"]:openApplication("textbox", creationMenu)
end)
...
AddEventHandler("np-business:client:removeStash", function (pData, pEntity)
  ...
  local deleted = exports["np-objects"]:DelObject(objInfo.id)
  if not deleted then
...
AddEventHandler("np-business:client:changeStashModel", function (pData, pEntity)
  ...
  exports["np-interface"]:openApplication("textbox", menuData)
end)

RegisterInterfaceCallback("np-business:client:updateStashModel", function(pData, pCb) 
  ...
  local updated = exports["np-objects"]:UpdateObject(pData.key.id, stashProp)
end)

RegisterInterfaceCallback("np-business:client:createStash", function(pData, pCb) 
  pCb({ data = {}, meta = { ok = true, message = "done" } })
  exports["np-interface"]:closeApplication("textbox")
