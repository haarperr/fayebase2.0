...
function setCoords(x, y) {
    $.post('https://np-dispatch_new/snp-coords', JSON.stringify({
        x: x,
        y: y
    }));
}
...
$(document).ready(function() {
	window.addEventListener('message', function(event){
		if (event.data.action == "open") {
            ...
            $('.mesai-allah-yok-EMS').html(createTableEMS(table));
            
            createTablePolice2(table);
            createTableEMS2(table);

            ilkSilme();
            
            $('.police-list').html(createPolice(table));
            $('.ems-list').html(createEMS(table));

            outAc();
            $('.called-list-inner').html(createPolice3(table));
            $('.called-list-inner').html(createEMS3(table));


            
            $('.called-list-inner').prepend(`
            <li class="MuiButtonBase-root MuiListItem-root MuiMenuItem-root MuiMenuItem-gutters MuiListItem-gutters MuiListItem-button snp-gps" tabindex="-1" role="option" aria-disabled="false" data-value="10">
            Set GPS Location<span class="MuiTouchRipple-root">
            </span></li>`)
            $('.snp-gps').click(function() {
                $.post('https://np-dispatch_new/snp-coords', JSON.stringify({
                    number: roninNumber
                }));
                $('.called-list').fadeOut();
            ...
            $('.snp-gps-location').click(function() {
                $.post('https://np-dispatch_new/snp-coords', JSON.stringify({
                    number: roninNumber
                }));
        ...
        if (event.data.action == "close") {
            $.post('https://np-dispatch_new/close', JSON.stringify({}));
            outKapa();
		}

        if (event.data.action == "close-callsign") {

            $.post('https://np-dispatch_new/close-callsign', JSON.stringify({}));
            outKapa();
		}
        ...
        $('.create-call').click(function() {
            $.post('https://np-dispatch_new/create_call', JSON.stringify({
                number: roninNumber
            }));
        })
        ...
        $('.dismiss-pingg').click(function() {
            createCall3.addClass('dismissed');
            $('.called-list').fadeOut();
            $.post('https://np-dispatch_new/dismiss_call-2', JSON.stringify({
                number: roninNumber
            }));
        })
    ...
    function createTableEMS(result) {
        var html = "";
        for(var k in result) {
            if (result[k].whounder == false) {
                html += `
    ...
    function vehiclesClick() {
        $('.vehicles').click(function() {
            ...
            if (value == 1) {
                ...
                $('.mesai-allah-yok').append(EMSCreat2(policeName, icon));
                removeItem()
                mesaiHold()
                $.post('https://np-dispatch_new/change-vehicle', JSON.stringify({
                    name: policeName,
                    icon: icon
                }));
            } else {
                ...
                $.post('https://np-dispatch_new/table_ayril', JSON.stringify({
                    name: remove
                }));
    ...
    $('.dismiss-ping').click(function() {
        ...
        $.post('https://np-dispatch_new/dismiss_call-1', JSON.stringify({
            number: roninNumber
        }));
    })
    ...
    function assignButtons(){
        $('.assing-buttons').click(function() {
            $.post('https://np-dispatch_new/add_police-assign', JSON.stringify({
    ...
    $('.crate-dismiss').click(function() {
        ...
        $.post('https://np-dispatch_new/create_call-2', JSON.stringify({
            number: roninNumber
        }));
    })
    ...
    $('.remove-dismis-ping').click(function() {
        $('.dismiss-list').fadeOut();
        createCall4.remove();
        $.post('https://np-dispatch_new/remove-call', JSON.stringify({
            number: roninNumber
        }));
    })
    ...
    function cureungotunu() {
        ...
        $('.remove-unit').click(function() {
            ...
            $.post('https://np-dispatch_new/delete', JSON.stringify({
                name: player
            }));
            ...
            $('.mesai-personel').each(function() {
                if ($.trim($(this).text()) == text) {
                    ...
                    if (owner == player) {
                        ...
                        $.post('https://np-dispatch_new/under-police', JSON.stringify({
                            player: player,
                            owner: owner
                        }));
                    }
    ...
    function callCameRemove(){
        $('.call-came').dblclick(function(e){
            ...
            $('.remove-list .MuiPaper-rounded ul li').click(function(){
                $.post('https://np-dispatch_new/delete_police-assign', JSON.stringify({
                    ronin: callCame
                }));
            })
        })
    ...
    function save(){
        ...
        $.post('https://np-dispatch_new/save', JSON.stringify({
            html: save
        }));
    ...
    function createTableEMS2(result) {
        var html = "";
        for(var k in result) {
            if (result[k].aktif == true) {
                html += 
