...
RPC.register("np-racing:getAllRaces", function(pSrc)
    local src = pSrc
...
RegisterNetEvent('np-racing:allRaces')
AddEventHandler('np-racing:allRaces', function()
    local allRaces = {
        races = Races,
        pendingRaces = PendingRaces,
        activeRaces = ActiveRaces,
...
RPC.register("np-racing:awardPlayer", function(pSrc, pAmt)
...
Citizen.CreateThread(function()
    ...
    Wait(100)
    TriggerEvent('np-racing:allRaces')
end)

RPC.register('np-racing:setAlias', function(src, usedItemIds, usedItemSlots, usedItemMetadatas, aliass)
...
RPC.register('np-racing:giveRacingUSB', function(pSource)
