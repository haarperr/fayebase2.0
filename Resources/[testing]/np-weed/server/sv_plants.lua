...
--     end)
    ...
    Citizen.CreateThread(function()
        ...
        TriggerClientEvent("np-weed:trigger_zone", -1, 1, weedplants)
        end
    end)
    
    RPC.register("np-weed:getPlants", function(pSource)
        local plants = Await(SQL.execute("SELECT * FROM weed_plants"))
        ...
        TriggerClientEvent("np-weed:trigger_zone", pSource, 1, weedplants)
    
        return weedplants
    end)
    
    RPC.register("np-weed:plantSeed", function(pSource, pCoords, pTypeMod)
    ...
    RPC.register("np-weed:addWater", function(pSource, pData)
    ...
    RPC.register("np-weed:addFertilizer", function(pSource, pData)
        local data = pData.param
    ...
    RPC.register("np-weed:harvestPlant", function(pSource, pId, pStrain)
        local time = os.time()
    ...
    RPC.register("np-weed:removePlant", function(pSource, pData, pFertilizer)
        local data = pData.param
    ...
    RPC.register("np-weed:addMaleSeed", function(pSource, pData)
        local data = pData.param
