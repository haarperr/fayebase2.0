...
RegisterNetEvent('np-weed:package_smallbud')
AddEventHandler('np-weed:package_smallbud', function()
    if exports['np-inventory']:hasEnoughOfItem('smallbud', 10) then
        if not pStartedPackaging then
            pStartedPackaging = true
            local finished = exports['np-taskbar']:taskBar(10000, 'Preparing')
...
RegisterNetEvent('np-weed:grab_package')
AddEventHandler('np-weed:grab_package', function()
...
RegisterNetEvent('np-weed:get_delivery')
AddEventHandler('np-weed:get_delivery', function()
    if pDeliverAmt == 3 then
        pDeliverAmt = 0
        inWeedRun = false
        pSentNoti = false
        exports["np-phone"]:DoPhoneNotification("home-screen", "CURRENT", 'Run complete.', 1)
        return
    end
    if inWeedRun then
    ...
    inWeedRun = true
    exports["np-phone"]:DoPhoneNotification("home-screen", "CURRENT", 'Drive to the delivery location with the goods.', 1)
    ...
    TriggerEvent('np-weed:create_ped')
            ...
            if inWeedRun then
                ...
                if distance < 40 then
                    if not pSentNoti then
                        exports["np-phone"]:DoPhoneNotification("home-screen", "CURRENT", 'Deliver the package.', 1)
                        pSentNoti = true
                        pCanDeliver = true
                    end
                end
...
RegisterNetEvent("np-weed:create_ped")
AddEventHandler("np-weed:create_ped", function()
    Citizen.Wait(5000)
    ...
    if not CurrentDeliveryNPC then
        ...
        exports["np-npcs"]:UpdateNPCData(CurrentDeliveryNPC.id, "position", {
            coords = coords,
            heading = DropOffsClose[rnd]["h"]
        })
...
AddEventHandler("np-weed:deliver_package", function()
    ...
    local finished = exports["np-taskbar"]:taskBar(20000, "Weighing Package", false, true, false, false, nil, 5.0, PlayerPedId())
    if finished == 100 then
    PlayAmbientSpeech1(CurrentDeliveryNPC.entity, "Chat_State", "Speech_Params_Force")
    Citizen.Wait(200)
    local finished = exports['np-taskbar']:taskBar(15000, "Counting Bills")
        if finished == 100 then
        RPC.execute("np-weed:deliverpackage")
        pCanDeliver = false
        Wait(1000)
        TriggerEvent("np-weed:drop_weed_package", 0.5)
        ...
        TriggerEvent('np-weed:get_delivery')
        end
    end
end)
...
AddEventHandler("np-weed:drop_weed_package", function(pRandomChance)
    local payment = 0
