            ...
            if hit then
                ...
                for matHash,matType in pairs(MaterialHashes) do
                    if materialHash == matHash then
                        ...
                        local finished = exports["np-taskbar"]:taskBar(15000, 'Planting Seed', false, true, false, false, nil, 5.0, PlayerPedId())
...
RegisterInterfaceCallback('np-weed:addWater', function (data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
    if exports['np-inventory']:hasEnoughOfItem('water', 1) then
        playPourAnimation()
        FreezeEntityPosition(PlayerPedId(), true)
        local finished = exports["np-taskbar"]:taskBar(5000,'Adding Water', false, true, false, false, nil, 5.0, PlayerPedId())
        ClearPedTasks(PlayerPedId())
        if finished == 100 then
            FreezeEntityPosition(PlayerPedId(), false)
            local success = RPC.execute("np-weed:addWater", data.key)
            if not success then
...
RegisterInterfaceCallback('np-weed:addFertilizer', function (data, cb)
    if exports['np-inventory']:hasEnoughOfItem('fertilizer', 1) then
        cb({ data = {}, meta = { ok = true, message = '' } })
        playPourAnimation()
        FreezeEntityPosition(PlayerPedId(), true)
        local finished = exports["np-taskbar"]:taskBar(5000, 'Adding Fertilizer', false, true, false, false, nil, 5.0, PlayerPedId())
        ClearPedTasks(PlayerPedId())
        if finished == 100 then
            FreezeEntityPosition(PlayerPedId(), false)
            local success = RPC.execute("np-weed:addFertilizer", data.key)
            if not success then
...
RegisterInterfaceCallback('np-weed:addMaleSeed', function (data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
    TaskStartScenarioInPlace(PlayerPedId(), "WORLD_HUMAN_GARDENER_PLANT", 0, true)
    FreezeEntityPosition(PlayerPedId(), true)
    local finished = exports["np-taskbar"]:taskBar(15000, 'Adding Male Seed', false, true, false, false, nil, 5.0, PlayerPedId())
    ClearPedTasks(PlayerPedId())
    if finished == 100 then
        FreezeEntityPosition(PlayerPedId(), false)
        RPC.execute("np-weed:addMaleSeed", data.key)
...
RegisterInterfaceCallback('np-weed:removePlant', function (data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
    ...
    if finished == 100 then
        ...
        local success = RPC.execute("np-weed:removePlant", data.key, getFertilizer)
        if not success then
...
AddEventHandler("np-weed:pickPlant", function(pContext, pEntity)
    ...
    if finished == 100 then
        ...
        RPC.execute("np-weed:harvestPlant", plantId, pStrain)
    else
        FreezeEntityPosition(PlayerPedId(), false)
    end
end)
...
AddEventHandler("np-polyzone:enter", function(zone, data)
    if zone == "np-weed:weed_zone" then
        inZone = inZone + 1
        if inZone == 1 then
            setDeleteAll = false -- remove
            RPC.execute("np-weed:getPlants")
        end
    end
end)
...
RegisterNetEvent('np-weed:trigger_zone')
AddEventHandler("np-weed:trigger_zone", function (type, pData)
    --Update all plants
    if type == 1 then
...
function getPlantById(pPlantId)
    local plants = RPC.execute("np-weed:getPlants")
...
function showPlantMenu(pPlantId)
    ...
    if growth < PlantConfig.HarvestPercent then
        context[#context+1] = {
            ...
            disabled = not exports["np-inventory"]:hasEnoughOfItem("water", 1, false),
        }

        context[#context+1] = {
            title  = 'Add Fertilizer',
            disabled = not exports["np-inventory"]:hasEnoughOfItem("fertilizer", 1, false),
            description = "",
            children = {
                {
    ...
    exports['np-interface']:showContextMenu(context);
end