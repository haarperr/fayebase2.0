...
local MethStartCorner = {
  ...
  options = {
    ...
    isEnabled = function(pEntity, pContext)
      local hasMeth = exports['np-inventory']:hasEnoughOfItem('meth1g', 1, false, true)
...
Citizen.CreateThread(function()
  exports['np-interact']:AddPeekEntryByEntityType(MethCornerInteract.group, MethCornerInteract.data, MethCornerInteract.options)
  exports['np-interact']:AddPeekEntryByEntityType(MethStartCorner.group, MethStartCorner.data, MethStartCorner.options)
  exports['np-interact']:AddPeekEntryByEntityType(MethStopCorner.group, MethStopCorner.data, MethStopCorner.options)
end)

AddEventHandler('np-meth:startCornering', function(pContext, pEntity)
  ...
  local canCorner, message = RPC.execute('np-meth:startCorner', CurrentCornerCoords)
  ...
  Citizen.CreateThread(function()
    ...
    while CurrentlyCornering do
      ...
      if not foundPed then
        ...
        local result = RPC.execute('np-meth:cornerPed', coords, NetworkGetNetworkIdFromEntity(foundPed), NetworkGetNetworkIdFromEntity(CurrentCornerVehicle))
    end
...
AddEventHandler('np-meth:cornerSale', function(pContext, pEntity)
  ...
  local hasBaggies = exports['np-inventory']:hasEnoughOfItem('meth1g', 1, true)
  ...
  RPC.execute('np-meth:cornerSyncHandoff', CurrentCornerCoords, NetworkGetNetworkIdFromEntity(pEntity))
  ...
  local didSale = RPC.execute('np-meth:cornerSale', CurrentCornerCoords, NetworkGetNetworkIdFromEntity(pEntity), CurrentCornerZone)
  if didSale then
    PlayAmbientSpeech1(pEntity, 'Chat_State', 'Speech_Params_Force')
    local rndAmt = math.random(1, 3)
    if rndAmt == 1 and exports['np-inventory']:hasEnoughOfItem('meth1g', 1) then
        TriggerEvent('money:clean2')
        TriggerEvent('inventory:removeItem', 'meth1g', 1)
      elseif rndAmt == 2 and exports['np-inventory']:hasEnoughOfItem('meth1g', 3) then
        TriggerEvent('money:clean2')
        TriggerEvent('money:clean2')
        TriggerEvent('money:clean2')
        TriggerEvent('inventory:removeItem', 'meth1g', 1)
        TriggerEvent('inventory:removeItem', 'meth1g', 1)
        TriggerEvent('inventory:removeItem', 'meth1g', 1)
      elseif rndAmt == 3 and exports['np-inventory']:hasEnoughOfItem('meth1g', 2) then
        TriggerEvent('money:clean2')
        TriggerEvent('money:clean2')
        TriggerEvent('inventory:removeItem', 'meth1g', 1)
        TriggerEvent('inventory:removeItem', 'meth1g', 1)
...
AddEventHandler('np-meth:stopCornering', function(pContext, pEntity)
  TriggerEvent('DoLongHudText', 'No longer selling...')
  stopCornering()
end)

RegisterNetEvent('np-meth:addCorneredPed')
AddEventHandler('np-meth:addCorneredPed', function(pPed)
  local ped = NetworkGetEntityFromNetworkId(pPed)
  ActivePeds[ped] = false
end)

RegisterNetEvent('np-meth:cornerSyncHandoff')
AddEventHandler('np-meth:cornerSyncHandoff', function(pPed)
  local ped = NetworkGetEntityFromNetworkId(pPed)
...
RegisterNetEvent('np-meth:cornerPed')
AddEventHandler('np-meth:cornerPed', function(pPed, pCornerCoords, pVehicle)
  local ped = NetworkGetEntityFromNetworkId(pPed)
  ActivePeds[ped] = true
  UsedPeds[ped] = true
  if NetworkHasControlOfEntity(ped) then
...
RegisterNetEvent('np-meth:cleanCornerPeds')
AddEventHandler('np-meth:cleanCornerPeds', function()
  UsedPeds = {}
end)
