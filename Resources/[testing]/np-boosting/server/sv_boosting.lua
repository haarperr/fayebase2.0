...
RegisterServerEvent('np-boosting:removeGNE')
AddEventHandler('np-boosting:removeGNE', function(amount, cid)
    exports.oxmysql:execute('SELECT gne FROM boosting_users WHERE identifier = ?', {cid}, function(result)
...
RegisterServerEvent('np-boosting:pay_gne')
AddEventHandler('np-boosting:pay_gne', function()
    local src = source
    local user = exports["np-base"]:getModule("Player"):GetUser(src)
	local cid = user:getCurrentCharacter().id
    local gnes = GetGNE(cid)
...
RegisterServerEvent('np-boosting:giveGNE')
AddEventHandler('np-boosting:giveGNE', function(amount)
    local src = source
    local user = exports["np-base"]:getModule("Player"):GetUser(src)
	local cid = user:getCurrentCharacter().id
