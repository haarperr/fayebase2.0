            ...
            Citizen.CreateThread(function()
                while notEmailed do
                  ...
                  if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), setCoords) >= 100.0 then
                    ...
                    TriggerServerEvent('np-boosting:pay_gne')
                    notEmailed = false
                  end
                end
              end)
  ...
  if targetVehicle == 0 then
    if vehicleHandle ~= nil and vehicleHandle ~= 0 and IsEntityAVehicle(vehicleHandle) then
        ...
        if CustomMinigame() == true then
            ...
            if(GetVehicleNumberPlateText(vehicleHandle) == placa)then
            if isVin then
                exports['np-dispatch']:dispatchadd('10-99', 'Tracker Device Tampering - Grand Theft Auto', '227')
...
RegisterNetEvent("np-boosting:open:boostingApp")
AddEventHandler('np-boosting:open:boostingApp', function()
  TriggerServerEvent("boosting:getLevel")
  TriggerEvent("boosting:qbSpawn")
  TriggerEvent("open:boostingApp")
end)

RegisterNUICallback("np-boosting:closeBoostingApp", function(data, cb)
  DoTimer(false)
...
RegisterNUICallback("np-boosting:startContract", function(data, cb) 
  if not AlredyContract then
    if data.IsVin then
      ...
      if units + tonumber(data.props.ExtraVin) <= tonumber(gne) then
        if exports['np-inventory']:hasEnoughOfItem('trackerdisabler', 1) then
...
RegisterNUICallback("np-boosting:transferContract", function(data, cb) 
  sendAppEvent("RemoveContractInfo", {IdToRemove = data.props.id})
  TriggerServerEvent("boosting:TransferContract", data)
  cb({})
end)

RegisterNUICallback("np-boosting:updateTime", function(data, cb) 
...
RegisterNUICallback("np-boosting:DeclineContract", function(data, cb) 
  sendAppEvent("RemoveContractInfo", {IdToRemove = data.props.id})
...
RegisterNUICallback("np-boosting:cancelContract", function(data, cb) 
  sendAppEvent("RemoveContractInfo", {IdToRemove = data.props.id})
  sendAppEvent('SetDisableButtons', {enableButtons = false})
  TriggerServerEvent("boosting:CancelContract", data)
...
RegisterNUICallback("np-boosting:JoinQueue", function(data, cb) 
...
RegisterNUICallback("np-boosting:BennysItem", function(data, cb) 
  if pendingDelivery == false then
    if data.ToPay <= tonumber(gne) then
      TriggerServerEvent('np-boosting:removeGNE', data.ToPay, exports['isPed']:isPed('cid'))
...
AddEventHandler("aspect-boosting:client:UseComputer" , function()
  if exports['np-boosting']:pCanUseLaptop() then
    CanScratchVehicle = false
    RemoveBlip(dropZoneblip)
