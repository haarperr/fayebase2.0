    ...
    if (canChange) {
        if (hasTracker && clothingCategory == "accessory") {
            ...
            if (clothingCategory == "model") {
                ...
                if (buttonType == "item") {
                    NP.Clothing.ResetItemTexture(this, clothingCategory);
                }
            } else {
                if (buttonType == "item") {
...
NP.Clothing.ResetItemTexture = function(obj, category) {
...
NP.Clothing.ToggleChange = function(bool) {
    canChange = bool;
}

NP.Clothing.SetCurrentValues = function(clothingValues) {
    $.each(clothingValues, function(i, item) {
        let iteNPats = $(".clothing-menu-container").find('[data-type="' + i + '"]');

        if (i == "facemix") { // Added for special case with range sliders
            $('#shapeMix').val(item.shapeMix);
            $('#skinMix').val(item.skinMix);
        } else {
            let input = $(iteNPats).find('input[data-type="item"]');
            let texture = $(iteNPats).find('input[data-type="texture"]');
...
NP.Clothing.Open = function(data) {
    ...
    NP.Clothing.SetMaxValues(data.maxValues);
    NP.Clothing.SetCurrentValues(data.currentClothing);
...
NP.Clothing.SetMaxValues = function(maxValues) {
...
NP.Clothing.ResetValues = function() {
...
$(document).ready(function() {
    ...
    window.addEventListener('message', function(event) {
        switch (event.data.action) {
            case "Open":
                NP.Clothing.Open(event.data);
                break;
            case "Close":
                NP.Clothing.Close();
                break;
            case "updateMax":
                NP.Clothing.SetMaxValues(event.data.maxValues);
                break;
            case "toggleChange":
                NP.Clothing.ToggleChange(event.data.allow);
                break;
            case "ResetValues":
                NP.Clothing.ResetValues();
                break;
