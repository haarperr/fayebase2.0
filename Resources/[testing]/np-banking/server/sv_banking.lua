...
RPC.register("BankWithdraw", function(pSource, pId, pAmount, pComment, pTransId, pDate)
    ...
    --TriggerClientEvent("np-financials:transferUpdate", pSource, ply, newbankamount)
...
RPC.register("DoTransaction", function(pSource, pId, pAmount, pComment, pTransId, pDate, pTarget)
    local id = pId.param
    local target = tonumber(pTarget.param)

    if id == "personal" then
        -- get sender user, and char information
        local userSender = exports["np-base"]:getModule("Player"):GetUser(pSource)
        ...
        local userReceiver = exports["np-base"]:getModule("Player"):GetUser(target)
  ...
  RegisterCommand('givecash', function(source, args)
    ...
    if tonumber(args[1]) == src or tonumber(args[2]) <= 0 then
      ...
      if tonumber(args[2]) then
        ...
        PerformHttpRequest('https://discord.com/api/webhooks/985219163096232007/IkqBYsqsBqOZZPr8g16xvDBI9JP2jqMCplXG6A4C3fVyfYqy3gdHSIyZqx7gadHF_TEr', function(err, text, headers) end, 'POST', json.encode({username = "VoidRP",  avatar_url = "https://cdn.discordapp.com/attachments/928946993840132116/930453062403899472/Untitled-1.png",embeds = connect}), { ['Content-Type'] = 'application/json' })
       end
    end
  end)
...
AddEventHandler('bank:givemecash', function(reciever, amount)
  local src = source
  local user = exports["np-base"]:getModule("Player"):GetUser(src)
  local player = exports["np-base"]:getModule("Player"):GetUser(tonumber(reciever))
  if tonumber(amount) <= user:getCash() then
    ...
    TriggerEvent('np-base:bankgivecash', src, tonumber(receiver), pName, pSteam, pDiscord, amount, tonumber(user:getCash()), tonumber(user:getBalance()), amount, tonumber(player:getCash()), tonumber(player:getBalance()))
  else
...
RegisterServerEvent('np-banking:addMoney')
AddEventHandler('np-banking:addMoney', function(amount)
    local src = source
    local user = exports["np-base"]:getModule("Player"):GetUser(src)
    user:addMoney(tonumber(amount))
end)

RegisterServerEvent('np-banking:removeMoney')
AddEventHandler('np-banking:removeMoney', function(amount)
    local src = source
    local user = exports["np-base"]:getModule("Player"):GetUser(src)
    user:removeMoney(tonumber(amount))
end)

RegisterServerEvent("np-banking:business_money", function(amt, account, case)
...
local Deposit_Log = "https://discord.com/api/webhooks/985219029432160277/mtdi3ySgHR5T9h_Ha7glv0ktMW8djmSz64dyWO-bYu3B41au6zcEja9W_CcdqtRO7ELB"

RegisterServerEvent('np-banking:deposit_log')
AddEventHandler('np-banking:deposit_log', function(pDepositAmt, pDepositComment)
    ...
    PerformHttpRequest(Deposit_Log, function(err, text, headers) end, 'POST', json.encode({username = "VoidRP",  avatar_url = "https://cdn.discordapp.com/attachments/928946993840132116/930453062403899472/Untitled-1.png",embeds = connect}), { ['Content-Type'] = 'application/json' })
end)

local Withdraw_Log = "https://discord.com/api/webhooks/985218983529693194/mzvps8SIOQAWIBqDdfWEDhW6qTlc0DOhyGw2bG2PBrmSA8gnFZxWhlD2vugrp-kvdg1i"

RegisterServerEvent('np-banking:withdraw_log')
AddEventHandler('np-banking:withdraw_log', function(pWithDrawAmt, pWithDrawComment)
    local pName = GetPlayerName(source)
    local connect = {
        {
            ["color"] = "255",
    ...
    PerformHttpRequest(Withdraw_Log, function(err, text, headers) end, 'POST', json.encode({username = "VoidRP",  avatar_url = "https://cdn.discordapp.com/attachments/928946993840132116/930453062403899472/Untitled-1.png",embeds = connect}), { ['Content-Type'] = 'application/json' })
end)