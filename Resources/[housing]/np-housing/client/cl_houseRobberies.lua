...
function attemptToLockPickHouse(skipPicking)
    ...
    if not skipPicking then
        if not Housing.housingBeingRobbedClient[propertyID] then
            ...
            local finished = exports["np-taskbar"]:taskBar(math.random(500), "Breaking in..")
            ...
            TriggerEvent("DoLongHudText","Cannot rob.",2)
            return
        end
    end
...
function canRobProperty()
    ...
    if daytime == true then -- Set default to true 
        return true
    end
...
function displayPickup(force,itemName)
    ...
    if Housing.currentlyDisplayingPickup == false and not force then
        ...
        if type(itemName) == "string" then
            exports["np-textui"]:showInteraction("[E] Take Object")
        else
            exports["np-textui"]:showInteraction("[E] Search Area")
        end
    end
end
...
function interactRob()
    ...
    if type(item) == "string" then
        ...
        TriggerEvent("animation:PlayAnimation","search")
        local finished = exports["np-taskbar"]:taskBar(15000,"Searcing Location")
        ClearPedTasks(PlayerPedId())
        if finished ~= 100 then
            return
        end
...
RegisterCommand('lockpickhouse', function()
