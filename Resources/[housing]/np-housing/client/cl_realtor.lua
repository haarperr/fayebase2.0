...
function setGarage()
    Housing.currentlyEditing = true
    
    if Housing.currentlyEditing == false then
        TriggerEvent("DoLongHudText", "Must be in edit mode !!", 2)
        return
    end
    ...
    if not IsPedInAnyVehicle(PlayerPedId(), false) then
        TriggerEvent("DoLongHudText", "Must not be in a vehicle.", 2)
        return
    end
...
function setInventory()
    ...
    if Housing.currentOwned[Housing.currentlyEditing] == nil then
        TriggerEvent("DoLongHudText", "You do not own this propery..", 2)
        return
    end
    ...
    if Housing.currentlyInsideBuilding then
        ...
        local finished = RPC.execute("updateCurrentSelected", Housing.currentlyEditing, Housing.currentHousingInteractions, Housing.hasEditedOrigin)
        if finished then
            TriggerEvent("DoLongHudText", "moved stash")
        end
    else
...
function placeBench(dropInventory)
    ...
    local playerCoords = GetEntityCoords(PlayerPedId())
    local objFound = GetClosestObjectOfType(playerCoords, 15.0, "gr_prop_gr_bench_02b", 0, 0, 0)
    print('objFound',json.encode(objFound))
    ...
    if objFound ~= 0 then
        ...
        if Housing.currentlyInsideBuilding then
            ...
            TriggerEvent("DoLongHudText", "Not near property", 2)
        end
    else
...
function openFurniture()
    ...
    if not hasCorrectFlags("canHaveFurniture", Housing.currentlyEditing) then
        TriggerEvent("DoLongHudText", "Unavailable for this property.", 2)
        return
    end
...
function isGov()
    ...
    if myjob == 'state' or myjob == 'police' or myjob == 'sheriff' or myjob == "judge" then
        return true
    end
    return false
end
...
function canPlaceInteractionPoint(nameIncoming,pos)
    ...
    if not canPlace then
        TriggerEvent("DoLongHudText", "Can't place the dot here, it might be too close to another dot.", 2)
    end
...
AddEventHandler("np-housing:buy", function(pPropertyId, pSeller)
    ...
    local tax = RPC.execute("PriceWithTaxString", "Goods")
...
AddEventHandler("np-housing:rent", function()
    ...
    exports["np-context22"]:showContext({
        {
            header = "Property",
            context = Housing.info[pPropertyId]["street"],
            event = ""
...
AddEventHandler("np-housing:buyed", function(pID , pTotal , pTax, pSeller)
    local complete, info = RPC.execute("np-housing:buy", pID, pTotal, pTax, pSeller)
...
AddEventHandler("np-housing:edit", function()
    ...
    local propertyId = ""
        ...
        TriggerEvent('DoLongHudText', 'Please close your phone and wait 1 moment..',2)
        Wait(3000)
        TriggerEvent('np-context:sendMenu', {
            {
                header = "Start Editing",
                context = "",
                event = "np-housing:editOptions",
                args = {
                    "enter",
                }
            },
...
AddEventHandler("np-housing:editOptions", function(pParams)
    local isComplete, propertyId, dist, zone = Housing.func.findClosestProperty()
    print(pParams)
    if pParams == "enter" then
        if not isComplete then
            TriggerEvent("DoLongHudText", propertyId, 2)
            return
        end
        enterEdit(propertyId)
        TriggerEvent('phone:close')
        TriggerEvent('DoLongHudText', 'Please wait 1 moment..')
    elseif pParams == "exit" then
        exitEdit(true)
    elseif pParams == "garage" then
        setGarage()
    elseif pParams == "stash" then
        setInventory()
    elseif pParams == "backdoor" then
        setBackdoor()
    elseif pParams == "wardrobe" then
        setCharChanger()
    elseif pParams == "crafting" then
        placeBench(true)
    elseif pParams == "furniture" then
        TriggerEvent('DoLongHudText', 'Please wait 1 moment',1)
        Citizen.Wait(4000)
        openFurniture()
    end
end)
...
AddEventHandler("housing:inventory", function()
    ...
    if max.canHaveInventory then
        if not lockdownCheck(propertyID) then
            TriggerEvent("DoLongHudText", "Property locked, you cannot open stash", 2)
            return
        end
        ...
        if Housing.currentOwned[propertyID] then
            ...
            local myJob = exports["isPed"]:isPed("myJob")
            if Housing.currentHousingLockdown[propertyID] and (myjob == 'state' or myjob == 'police' or myjob == 'sheriff' or  myjob == "judge") then
                Housing.func.OpenStash(propertyID,cat)
            end
        end
    end
...
AddEventHandler("housing:charLogout", function()
    ...
    if max.canHaveCharSelect then
        DoScreenFadeOut(500)
        exitingBuilding()
        TriggerServerEvent('np-base:ResetPlayerInfo')
        Citizen.Wait(1000)
...
RegisterCommand('house-blips', function()
    TriggerEvent('np-housing:blips')
end)
