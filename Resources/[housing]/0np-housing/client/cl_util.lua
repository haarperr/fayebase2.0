...
function canRent()
    ...
    if isComplete and dist <= 3.0 then print("something")
        if Housing.typeInfo[Housing.info[propertyId].model].cat == "buisness" then
            return false
        end
...
function hasKey(propertyID)
    local job = exports["np-base"]:CurrentJob()
    if job == 'state' or job == 'police' or job == 'sheriff' or job == "judge" then
        return true
    end
...
RegisterNetEvent('doClothing' ,function()
    TriggerServerEvent('np-clothingmenu:list_outfits')
end)
...
AddEventHandler("property:menuAction", function(pData)
    ...
    if isComplete and dist <= 3.0 then
        ...
        elseif action == "forfeit" then
            ...
            exports["np-context22"]:showContextMenu(MenuData["property_check"])
        end
    else
...
Citizen.CreateThread(function()
    exports["np-binds"]:registerKeyMapping("","Housing", "Interact", "+propertyInteract", "-propertyInteract", "E")
