...
RegisterInterfaceCallback("np-ui:raid_clothes:addOutfitPrompt", function(data, cb)
...
RegisterInterfaceCallback("np-ui:raid_clothes:addOutfit", function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
...
RegisterInterfaceCallback("np-ui:raid_clothes:changeOutfit", function(data, cb)
    cb({ data = {}, meta = { ok = true, message = 'done' } })
    TriggerEvent('hotel:outfit', { true, data.key }, 3)
    exports["np-ui"]:hideContextMenu()
end)

RegisterInterfaceCallback("np-ui:raid_clothes:deleteOutfit", function(data, cb)
    cb({ data = {}, meta = { ok = true, message = 'done' } })
...
AddEventHandler("np-polyzone:enter", function(zone, data)
    ...
    if currentZone then
        exports['np-interface']:showInteraction(("[M] %s"):format(currentZone.text))
...
RegisterInterfaceCallback("np-ui:raid_clothes:CustomaddOutfitPrompt", function(data, cb)
    cb({ data = {}, meta = { ok = true, message = 'done' } })
    Wait(1) --wait to fix ui bug?
    exports['np-ui']:openApplication('textbox', {
        callbackUrl = 'np-ui:raid_clothes:addCustomOutfit',
        key = data.key,
        items = {
            {
                icon = "pencil-alt",
...
RegisterInterfaceCallback("np-ui:raid_clothes:addCustomOutfit", function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
