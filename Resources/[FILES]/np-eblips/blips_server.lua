...
RegisterNetEvent('np-eblips:server:registerSourceName')
AddEventHandler('np-eblips:server:registerSourceName', function(source, name)
    if not source_cache[source] then source_cache[source] = { } end
    source_cache[source].name = name
end)

RegisterNetEvent('np-eblips:server:registerPlayerBlipGroup')
AddEventHandler('np-eblips:server:registerPlayerBlipGroup', function(source, group)
...
RegisterNetEvent('np-eblips:server:removePlayerBlipGroup')
AddEventHandler('np-eblips:server:removePlayerBlipGroup', function(source, group)
...
RegisterNetEvent('np-eblips:server:registerPlayerTempBlipGroup')
AddEventHandler('np-eblips:server:registerPlayerTempBlipGroup', function(source, group, time)
    TriggerEvent('np-eblips:server:registerPlayerBlipGroup', source, 'police')
    SetTimeout(time, function()
        TriggerEvent('np-eblips:server:removePlayerBlipGroup', source, 'police')
    end)
end)
            ...
            Citizen.CreateThread(function()
                for source, is_valid_member in pairs(blip.members) do
                    if is_valid_member then -- more sanity checking
                        TriggerClientEvent('np-eblips:client:syncMyBlips', source, blips_data)
                        Citizen.Wait(100)
                    end
                end
            end)
