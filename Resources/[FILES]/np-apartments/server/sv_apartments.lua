...
AddEventHandler("apartment:serverUnlockAprts",function(roomType)
    ...
    exports.oxmysql:execute("select * from character_motel WHERE cid=@cid",{
        ...
        if #result > 0 then
            if result[1].building_type == roomType then
                TriggerClientEvent("np-apps:unlocks1",src)
            else
                TriggerClientEvent("np-apps:unlocks2",src)
            end
        end
    end)
end)
...
RegisterServerEvent('np-apartments:SetNewRouting')
AddEventHandler('np-apartments:SetNewRouting', function(roomNumber)
    local src = source
    SetPlayerRoutingBucket(src, roomNumber)
end)

RegisterServerEvent('np-apartments:GetRouting')
AddEventHandler('np-apartments:GetRouting', function()
    local src = source
    GetPlayerRoutingBucket(src)
end)

RegisterServerEvent('np-apartments:SetOldRouting')
AddEventHandler('np-apartments:SetOldRouting', function()
    local src = source
