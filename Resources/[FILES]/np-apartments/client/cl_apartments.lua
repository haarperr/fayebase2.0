...
function Apart.enterMotel(roomNumber,roomType,isSpawn)
    ...
    TriggerServerEvent('np-apartments:SetNewRouting', roomNumber)
...
function Apart.processBuildType(numMultiplier,roomType,isSpawn)
    DoScreenFadeOut(1)
    TriggerServerEvent('np-apartments:GetRouting')
...
function Apart.leaveApartment()
    ...
    TriggerServerEvent('np-apartments:SetOldRouting')
    Citizen.Wait(100)
    TriggerServerEvent('np-apartments:GetRouting')
end
...
AddEventHandler('apartments:menuAction', function(action)
    ...
    elseif action == "forfeit" then
        -- exports["np-interface"]:showContextMenu(MenuData["apartment_check"])
    end
end)
