...
RegisterServerEvent('np-hospital:server:RequestBed')
AddEventHandler('np-hospital:server:RequestBed', function()
    for k, v in pairs(beds) do
        if not v.taken then
            v.taken = true
            bedsTaken[source] = k
            TriggerClientEvent('np-hospital:client:SendToBed', source, k, v)
            return
        end
    end
...
RegisterServerEvent('np-hospital:server:RPRequestBed')
AddEventHandler('np-hospital:server:RPRequestBed', function(plyCoords)
    ...
    for k, v in pairs(beds) do
        ...
        if distance < 3.0 then
            if not v.taken then
                v.taken = true
                foundbed = true
                TriggerClientEvent('np-hospital:client:RPSendToBed', source, k, v)
                return
            else
...
RegisterServerEvent('np-hospital:server:EnteredBed')
AddEventHandler('np-hospital:server:EnteredBed', function()
    local src = source
    ...
    TriggerClientEvent('np-hospital:client:FinishServices', src)
end)

RegisterServerEvent('np-hospital:server:LeaveBed')
AddEventHandler('np-hospital:server:LeaveBed', function(id)
    beds[id].taken = false
end)