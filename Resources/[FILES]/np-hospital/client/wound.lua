...
function CheckDamage(ped, bone, weapon)
    ...
    if Config.Bones[bone] ~= nil then
        if not BodyParts[Config.Bones[bone]].isDamaged then
            ...
            TriggerServerEvent('np-hospital:server:SyncInjuries', {
                limbs = BodyParts,
...
RegisterNetEvent('np-hospital:client:SyncBleed')
AddEventHandler('np-hospital:client:SyncBleed', function(bleedStatus)
    isBleeding = tonumber(bleedStatus)
    DoBleedAlert()
end)

RegisterNetEvent('np-hospital:client:FieldTreatLimbs')
AddEventHandler('np-hospital:client:FieldTreatLimbs', function()
    for k, v in pairs(BodyParts) do
        v.isDamaged = false
        v.severity = 1
    end
...
RegisterNetEvent('np-hospital:client:FieldTreatBleed')
AddEventHandler('np-hospital:client:FieldTreatBleed', function()
    if isBleeding > 1 then
...
RegisterNetEvent('np-hospital:client:ReduceBleed')
AddEventHandler('np-hospital:client:ReduceBleed', function()
    if isBleeding > 0 then
        isBleeding = tonumber(isBleeding) - 1
...
RegisterNetEvent('np-hospital:client:ResetLimbs')
AddEventHandler('np-hospital:client:ResetLimbs', function()
    for k, v in pairs(BodyParts) do
        v.isDamaged = false
        v.severity = 0
    end
...
RegisterNetEvent('np-hospital:client:RemoveBleed')
AddEventHandler('np-hospital:client:RemoveBleed', function()
    isBleeding = 0
