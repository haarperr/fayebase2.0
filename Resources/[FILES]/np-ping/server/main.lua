RegisterServerEvent("np-ping:attempt", function(pCoords, pFinderId)
	local pSrc = source
	...
	if pFinderId ~= nil then
        if pFinderId:lower() == 'accept' then
            TriggerClientEvent('np-ping:client:AcceptPing', pSrc)
        elseif pFinderId:lower() == 'reject' then
            TriggerClientEvent('np-ping:client:RejectPing', pSrc)
        else
            local tSrc = tonumber(pFinderId)
            if pSrc ~= tSrc then
                TriggerClientEvent('np-ping:client:SendPing', tSrc, name, pSrc, pCoords)
            else
...
RegisterServerEvent('np-ping:server:SendPingResult')
AddEventHandler('np-ping:server:SendPingResult', function(id, result)
