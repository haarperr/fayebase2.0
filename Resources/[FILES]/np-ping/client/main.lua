    ...
    Citizen.CreateThread(function()
        ...
        while isPending do
            ...
            if count >= Config.Timeout and isPending then
                ...
                TriggerEvent('np-hud:hide_ping')
                TriggerServerEvent('np-ping:server:SendPingResult', pendingPing.id, 'timeout')
                pendingPing = nil
                isPending = false
            end
            Citizen.Wait(1000)
...
RegisterNetEvent('np-ping:client:SendPing')
AddEventHandler('np-ping:client:SendPing', function(sender, senderId, pCoords)
    if pendingPing == nil then
        ...
        TriggerServerEvent('np-ping:server:SendPingResult', pendingPing.id, 'received')
        TriggerEvent('DoLongHudText', pendingPing.name .. ' Sent You a Ping, Use /ping accept To Accept', 1)
        TriggerEvent('np-hud:show_ping')
        isPending = true
        ...
        TriggerServerEvent('np-ping:server:SendPingResult', senderId, 'unable')
    end
end)

RegisterNetEvent('np-ping:client:AcceptPing')
AddEventHandler('np-ping:client:AcceptPing', function()
    if isPending then
        ...
        TriggerEvent('np-hud:hide_ping')
        TriggerServerEvent('np-ping:server:SendPingResult', pendingPing.id, 'accept')
        isPending = false
    else
...
RegisterNetEvent('np-ping:client:RejectPing')
AddEventHandler('np-ping:client:RejectPing', function()
    if pendingPing ~= nil then
        TriggerEvent('DoLongHudText', 'Rejected Ping From ' .. pendingPing.name, 2)
        TriggerServerEvent('np-ping:server:SendPingResult', pendingPing.id, 'reject')
        pendingPing = nil
        isPending = false
    else
        TriggerEvent('DoLongHudText', 'You Have No Pending Ping', 2)
...
RegisterNetEvent('np-ping:client:RemovePing')
AddEventHandler('np-ping:client:RemovePing', function()
    if pendingPing ~= nil then
        RemoveBlip(pendingPing.blip)
        pendingPing = nil
...
RegisterCommand('ping', function(src, args, raw)
    TriggerServerEvent("np-ping:attempt",  GetEntityCoords(PlayerPedId()), args[1])
end)
