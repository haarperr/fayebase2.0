...
AddEventHandler('paycheck:server:send', function(cid)
	...
	if user ~= false then
		...
		elseif job == "auto_bodies" or job == "tuner_shop" or job == "paleto_mech" or job == "auto_exotics" or job == "sea_side" or job == "car_shop" then -- Mechanic Shops
			TriggerEvent("paycheck:server:add", src, cid, 100)
...
AddEventHandler("paycheck:collect", function(cid)
	...
	exports['np-admin']:addPlayerLog('Job Pay', GetPlayerIdentifier(src, 0), 'User received paycheck, amount: '..cid, pCid, 'None')
end)