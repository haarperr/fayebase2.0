...
RegisterCommand('dev-set-music', function(source , args)

    local url = exports["np-applications"]:KeyboardInput({
...
RegisterNetEvent("vanilla:request:song", function()
    local job = exports["np-base"]:GroupRank('vanilla_unicorn')
    if job >= 1 then
        local url = exports["np-applications"]:KeyboardInput({
            header = "Place the song URL below.",
            rows = {
...
RegisterNetEvent("attemt:play:song", function()
    local pUrl = exports["np-applications"]:KeyboardInput({
        header = "Place the song URL below.",
        rows = {
            {
                id = 0,
    ...
    elseif pUrl[2].input then
        ...
        TriggerEvent("np-musicplayer:stateSound", "volume", data)
