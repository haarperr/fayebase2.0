...
RegisterNetEvent('np-mdt:RequestOpen')
AddEventHandler('np-mdt:RequestOpen', function()
	TriggerServerEvent('np-mdt:OpenMDT')
end)
...
--         local PlayerInfo = RPC.execute("np-mdt:getInfo")
...
--         SendNUIMessage({type = "data", name = "Welcome, " ..PlayerInfo.firstname..' '..PlayerInfo.lastname, location = playerStreetsLocation, fullname = PlayerInfo.firstname..' '..PlayerInfo.lastname})
    ...
    -- AddEventHandler('np-mdt:getActiveUnits', function(lspd, bcso, sasp, ems)
    ...
    RegisterNUICallback("deleteBulletin", function(data, cb)
        local ignoreId, sentData, job = RPC.execute("np-mdt:deleteBulletin", data.id)
    ...
    RegisterNUICallback("updateLicence", function(data, cb)
        ...
        local cid = data.cid
        RPC.execute("np-mdt:updateLicence", type, status, cid)
    end)
    RegisterNUICallback("newBulletin", function(data, cb)
        local ignoreId, sentData, job = RPC.execute("np-mdt:newBulletin", data.title, data.info, data.time)
        if ignoreId == GetPlayerServerId(PlayerId()) then 
            return 
        end
        if job == "police" or job == "state" or job == "sheriff" and exports['isPed']:isPed("myjob") == "police" or exports['isPed']:isPed("myjob") == "state" or exports['isPed']:isPed("myjob") == "sheriff" then
    ...
    AddEventHandler('np-mdt:open', function(job, lastname, firstname)
        open = true
        EnableGUI(open)
        local x, y, z = table.unpack(GetEntityCoords(PlayerPedId()))
        local currentStreetHash, intersectStreetHash = GetStreetNameAtCoord(x, y, z)
    ...
    RegisterNUICallback("searchProfiles", function(data, cb)
        SendNUIMessage({type = "profiles", data = RPC.execute("np-mdt:searchProfile", data.name), isLimited = RPC.execute("np-mdt:isLimited")})
    end)
    RegisterNUICallback("saveProfile", function(data, cb)
        RPC.execute("np-mdt:saveProfile", data.pfp, data.description, data.id)
    end)
    RegisterNUICallback("newWeapon", function(data, cb)
        RPC.execute("np-mdt:addWeapon", data.id, data.serialnumber)
    end)
    RegisterNUICallback("getProfileData", function(data, cb)
        local isLimited = RPC.execute("np-mdt:isLimited")
    ...
    RegisterNUICallback("newTag", function(data, cb)
        if data.id ~= "" and data.tag ~= "" then
            RPC.execute("np-mdt:newTag", data.id, data.tag)
        end
    end)
    RegisterNUICallback("removeProfileTag", function(data, cb)
        RPC.execute("np-mdt:removeProfileTag", data.cid, data.text)
    end)
    RegisterNUICallback("addGalleryImg", function(data, cb)
        RPC.execute("np-mdt:addGalleryImg", data.cid, data.URL)
    end)
    RegisterNUICallback("removeGalleryImg", function(data, cb)
        RPC.execute("np-mdt:removeGalleryImg", data.cid, data.URL)
    end)
    RegisterNUICallback("searchIncidents", function(data, cb)
        SendNUIMessage({type = "incidents", data = RPC.execute("np-mdt:searchIncidents", data.incident)})
    end)
    ...
    RegisterNUICallback("incidentSearchPerson", function(data, cb)
        SendNUIMessage({type = "incidentSearchPerson", data = RPC.execute("np-mdt:incidentSearchPerson", data.name)})
    end)
    RegisterNUICallback("searchBolos", function(data, cb)
        SendNUIMessage({type = "bolos", data = RPC.execute("np-mdt:searchBolos", data.searchVal)})
    end)
    RegisterNUICallback("getAllBolos", function(data, cb)
        SendNUIMessage({type = "bolos", data = RPC.execute("np-mdt:getAllBolos")})
    end)
    RegisterNUICallback("getAllIncidents", function(data, cb)
        SendNUIMessage({type = "incidents", data = RPC.execute("np-mdt:getAllIncidents")})
    end)
    RegisterNUICallback("getBoloData", function(data, cb)
        SendNUIMessage({type = "boloData", data = RPC.execute("np-mdt:getBoloData", data.id)})
    end)
    RegisterNUICallback("jailPlayer", function(data, cb)
        RPC.execute("np-mdt:JailPlayer", data.cid, data.time)
    end)
    RegisterNUICallback("newBolo", function(data, cb)
        local sentData = RPC.execute("np-mdt:newBolo", data.existing, data.id, data.title, data.plate, data.owner, data.individual, data.detail, data.tags, data.gallery, data.officers, data.time)
        SendNUIMessage({type = "boloComplete", data = sentData})
    end)
    RegisterNUICallback("deleteBolo", function(data, cb)
        RPC.execute("np-mdt:deleteBolo", data.id)
    end)
    RegisterNUICallback("deleteIncident", function(data, cb)
        RPC.execute("np-mdt:deleteIncident", data.id, data.time)
    end)
    RegisterNUICallback("deleteReport", function(data, cb)
        RPC.execute("np-mdt:deleteReport", data.id, data.time)
    end)
    RegisterNUICallback("deleteICU", function(data, cb)
        RPC.execute("np-mdt:deleteICU", data.id)
    end)
    RegisterNUICallback("getAllReports", function(data, cb)
        local asdsadassa = RPC.execute("np-mdt:getAllReports")
        print(json.encode(asdsadassa))
        SendNUIMessage({type = "reports", data = RPC.execute("np-mdt:getAllReports")})
    end)
    RegisterNUICallback("getReportData", function(data, cb)
        SendNUIMessage({type = "reportData", data = RPC.execute("np-mdt:getReportData", data.id)})
    end)
    RegisterNUICallback("searchReports", function(data, cb)
        SendNUIMessage({type = "reports", data = RPC.execute("np-mdt:searchReports", data.name)})
    end)
    RegisterNUICallback("newReport", function(data, cb)
        local sentData = RPC.execute("np-mdt:newReport", data.existing, data.id, data.title, data.type, data.detail, data.tags, data.gallery, data.officers, data.civilians, data.time)
    ...
    RegisterNUICallback("saveVehicleInfo", function(data, cb)
        RPC.execute("np-mdt:saveVehicleInfo", data.plate, data.imageurl, data.notes)
    end)
    RegisterNUICallback("saveWeaponInfo", function(data, cb)
        RPC.execute("np-mdt:saveWeaponInfo", data.serialnumber, data.imageurl, data.brand, data.type, data.notes)
    end)
    RegisterNUICallback("knownInformation", function(data, cb)
        RPC.execute("np-mdt:knownInformation", data.type, data.status, data.plate)
    end)
    RegisterNUICallback("getAllLogs", function(data, cb)
        SendNUIMessage({type = "getAllLogs", data = RPC.execute("np-mdt:getAllLogs")})
    end)
    RegisterNUICallback("getPenalCode", function(data, cb)
        local titles, penalcode = RPC.execute("np-mdt:getPenalCode")
        SendNUIMessage({type = "getPenalCode", titles = titles, penalcode = penalcode})
    end)
    -- RegisterNUICallback("missingCitizen", function(data, cb)
    --     RPC.execute("np-mdt:missingCitizen", data.cid, data.bool)
    -- end)
    ...
    --     RPC.execute("np-mdt:deleteMissing", data.id, data.time)
    -- end)
    ...
    --     RPC.execute("np-mdt:saveMissingInfo", data.id, data.imageurl, data.notes)
    -- end)
    RegisterNUICallback("toggleDuty", function(data, cb)
        RPC.execute("np-mdt:toggleDuty", data.cid, data.status)
    end)
    ...
    RegisterNUICallback("saveIncident", function(data, cb)
        RPC.execute("np-mdt:saveIncident", data.ID, data.title, data.information, data.tags, data.officers, data.civilians, data.evidence, data.associated, data.time)
    end)
    RegisterNUICallback("removeIncidentCriminal", function(data, cb)
        RPC.execute("np-mdt:removeIncidentCriminal", data.cid, data.incidentId)
    end)
    RegisterNUICallback("setWaypoint", function(data, cb)
        local callInformation = RPC.execute("np-mdt:setWaypoint", data.callid)
    ...
    RegisterNUICallback("callDetach", function(data, cb)
        local callid, sentData = RPC.execute("np-mdt:callDetach", data.callid)
    ...
    RegisterNUICallback("removeCall", function(data, cb)
        local src, callid = RPC.execute("np-mdt:removeCall", data.callid)
    ...
    RegisterNUICallback("callAttach", function(data, cb)
        local callid, sentData = RPC.execute("np-mdt:callAttach", data.callid)
        local job = exports['isPed']:isPed("myjob")
        if job == "police" or job == "state" or job == "sheriff" or job == 'ems' then
    ...
    RegisterNUICallback("attachedUnits", function(data, cb)
        local sentData, callid = RPC.execute("np-mdt:attachedUnits", data.callid)
        SendNUIMessage({type = "attachedUnits", data = sentData, callid = callid})
    end)
    RegisterNUICallback("callDispatchDetach", function(data, cb)
        RPC.execute("np-mdt:callDispatchDetach", data.callid, data.cid)
    end)
    RegisterNUICallback("setDispatchWaypoint", function(data, cb)
        local callInformation = RPC.execute("np-mdt:setDispatchWaypoint", data.callid)
        SetNewWaypoint(callInformation['origin']['x'], callInformation['origin']['y'])
    end)
    RegisterNUICallback("callDragAttach", function(data, cb)
        RPC.execute("np-mdt:callDragAttach", data.callid, data.cid)
    end)
    RegisterNUICallback("setWaypointU", function(data, cb)
        local sentData = RPC.execute("np-mdt:setWaypoint:unit", data.cid)
        SetNewWaypoint(sentData.x, sentData.y) 
    end)
    -- RegisterNUICallback("dispatchMessage", function(data, cb)
    --     local sentData = RPC.execute("np-mdt:sendMessage", data.message, data.time)
    --     local job = exports['isPed']:isPed("myjob")
    --     if job == "police" or job == "state" or job == "sheriff" or job == 'ems' then
    ...
    --     SendNUIMessage({type = "dispatchmessages", data = RPC.execute("np-mdt:refreshDispatchMsgs")})
    -- end)
-- end
