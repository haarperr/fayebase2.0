...
RegisterNUICallback('racing-app', function(data)
  -- RPC.execute('np-phone:getRaceTrack')
  local distance = 0.0
  local rank = exports["isPed"]:GroupRank("ug_racing")
  local complete = RPC.execute('np-phone:getRaceTrack')
  -- TriggerServerEvent('np-phone:getRaceTrack')
  local raceTName = {}
  for i,v in pairs(complete) do
    for j,k in pairs(v.checkpoints) do
      if k.start == true then
        local tname = v.track_name
        -- local distance = string.format("%.2f %%",v.distance*0.00014)
        -- print("TDIST",Round(v.distance*0.00014,2),distance,math.ceil(v.distance*0.00014))
        table.insert(raceTName, {
          id = v.id,
          name = tname,
          dist = Round(v.distance*0.00014,2),
          x = k.x,
          y = k.y,
          z = k.z
        })
