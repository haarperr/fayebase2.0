...
RegisterNetEvent("np-hud:EnableHud", function(status)
    isLoggedIn = status
    if status == true then
        exports['np-interface']:showInteraction('[F] To pay respects')
        HudActive = false
    end
end)
...
RegisterNetEvent("np-weapons:client:showWeaponFireRate", function (pBool)
    ...
    while true do 
        ...
        if isLoggedIn then
            ...
            SendNUIMessage({
                ...
                gps = gps,
                gpu = gpu,
                -- nos = exports['np-vehicles']:nitroLevel(),
                dev = developer,
                debug = debug,
...
RegisterNetEvent('np-hud:pursuitModeHUD')
AddEventHandler('np-hud:pursuitModeHUD', function(value)
...
RegisterNUICallback("SaveSettings", function(data)
    ...
    TriggerEvent("np-phone:setSomeShit",bg,model,twatNotify,callNotify)
    local circleTaskbar = GetResourceKvpInt(cid..'-circleTaskbar')
    if circleTaskbar == 1 then
        TriggerEvent("np-preferences:setPreferences", true)
    elseif circleTaskbar == 0 then
        TriggerEvent("np-preferences:setPreferences", false)
    end
...
AddEventHandler('np-preferences:setPreferences', function(bool)
    local data = {
        ["hud.misc.circle.taskbar.enabled"] = bool
    }
    exports["np-interface"]:sendAppEvent('preferences', data)
end)
...
RegisterNetEvent("np-hud:SetValues", function()
    ...
    local model = exports['np-phone']:getModelForHud()
    local background = exports['np-phone']:getBackgroundForHud()
    local twat = exports['np-phone']:getTwatForHud()
    local call = exports['np-phone']:getCallForHud()
...
RegisterNetEvent('np-hud:boosting_value')
AddEventHandler('np-hud:boosting_value', function(pValue)
