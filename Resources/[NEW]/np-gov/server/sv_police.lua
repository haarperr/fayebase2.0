RPC.register("np-gov:police:showBadge", function(pSource, pInfo, pCoords)
    print("[np-gov] [sv_police] [RPC] np-gov:police:showBadge")
    local info = json.decode(pInfo.param)
    print("[np-gov] [sv_police] [RPC] np-gov:police:showBadge - info: " .. json.encode(info))
    local playerCoords = pCoords.param
    print("[np-gov] [sv_police] [RPC] np-gov:police:showBadge - playerCoords: " .. json.encode(playerCoords))
    local players = GetNearbyPlayers(playerCoords, 10)
    print("[np-gov] [sv_police] [RPC] np-gov:police:showBadge - players: " .. json.encode(players))

    for _, serverID in ipairs(players) do
        TriggerClientEvent("np-gov:police:showBadge", serverID, pSource, info)
    end
end)

RPC.register("np-gov:police:getBadge", function(pSource)
    local src = pSource
