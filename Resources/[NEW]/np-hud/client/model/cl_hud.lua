...
RegisterNUICallback('np-hud:hudSetPreferences', function(data, cb)
...
AddEventHandler('np-preferences:setPreferences', function(pData)
    exports["np-interface"]:sendAppEvent('preferences', pData)
end)
...
AddEventHandler("np-hud:watch", function()
...
AddEventHandler("np-hud:watch:reset", function()
...
AddEventHandler("np-hud:changeRange", function(proximity)
    sendAppEvent("hud", {
        voiceRange = tonumber(proximity)
    })
end)
...
Citizen.CreateThread(function()
    ...
    while true do 
        ...
        if isLoggedIn then
            ...
            local pRadio = exports['np-voice']:RadioCheck()
...
function getFuel()
    fuel = exports["np-vehicles"]:CurrentFuel() or 50
end
    ...
    Citizen.CreateThread(function()
        while engineOn do
            ...
            if GetPedInVehicleSeat(veh, -1) == PlayerPedId() then
                harnessDurability = 100 --exports["np-vehicles"]:GetVehicleMetadata(veh, 'harness')
            end
        ...
        if (IsPedInAnyVehicle(player, false)) then
            ...
            if GetPedInVehicleSeat(veh, -1) == player then
                ...
                if not DivingStatus then
                    ...
                    if Fuel > 105 then
                        --Fuel = DecorGetInt(veh, "GetVehicleCurrentFuel")
                        Fuel = exports["np-vehicles"]:GetVehicleFuel(veh)
                    end                     
                    if Fuel == 101 then
                        --Fuel = DecorGetInt(veh, "GetVehicleCurrentFuel")
                        Fuel = exports["np-vehicles"]:GetVehicleFuel(veh)
                ...
                if ( setLastUpdate > 300) then
                    --DecorSetInt(veh, "GetVehicleCurrentFuel", fRound(Fuel))
                    exports["np-vehicles"]:CurrentFuel(veh, Fuel)
                    lasteupdate = 0
                end
