...
var ShowroomFunctions = {
    ...
    GetCars: function () {
        $.post('https://np-showroom/np-ui:showroomChangeCar', JSON.stringify({ model: cars[selectedId].model }), function (cb) {
...
$(document).ready(function () {
    ...
    window.addEventListener('message', (event) => {
        ...
        if (data.meta.message = "done") {
            ...
            $.post('https://np-showroom/np-ui:application-closed', JSON.stringify({ name: "showroom" }));
        }
    })
});
...
$(document).on("click", "#purchase", function () {
    ...
    $(".showroom-wrapper").hide();
    $.post('https://np-showroom/np-ui:application-closed', JSON.stringify({ name: "showroom" }));
    $.post('https://np-showroom/showroomPurchaseCurrentVehicle', JSON.stringify({ model: cars[selectedId].model, price:cars[selectedId].price, zoneName: zone }), function(cb){
     if( cb.meta.ok) {
        ShowroomFunctions.Close()
        canPress = false
     } else {
...
$(document).on("click", "#testdrive", function () {
    ...
    $(".showroom-wrapper").hide();
    $.post('https://np-showroom/np-ui:application-closed', JSON.stringify({ name: "showroom" }));
    $.post('https://np-showroom/showroomTestDriveCurrentVehicle', JSON.stringify({ model: cars[selectedId].model, price:cars[selectedId].price, zoneName: zone }), function(cb){
     if( cb.meta.ok) {
        ShowroomFunctions.Close()
        canPress = false
     } else {
