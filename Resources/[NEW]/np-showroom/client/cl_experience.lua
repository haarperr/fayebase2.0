...
RegisterNUICallback("np-ui:application-closed", function(data, cb)
  ...
  Citizen.CreateThread(function()
    ...
    DeleteEntity(vehicle)
	TriggerServerEvent('np-showrooms:exit_instance')
  end)
end)
...
AddEventHandler("np-showroom:enterExperience", function()  
  openShowroom("pdm")
  SetNuiFocus(true, true)
end)

RegisterNetEvent('np-tunercatty')
AddEventHandler('np-tunercatty', function()
	local InstanceData = math.random(1, 1000)

	if exports['np-business']:IsEmployedAt('art_gallery') then
		TriggerServerEvent('np-showrooms:enter_instance', InstanceData)
		TriggerEvent('np-showroom:enterExperience')
	else
