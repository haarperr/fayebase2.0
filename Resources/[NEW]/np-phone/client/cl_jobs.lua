...
AddEventHandler("signIntoJob", function(job)
  if signedin then exports['np-phone']:SendAlert('error', 'You already signed in', 3000) return end
  signedin = true
  signedinjob = job
  local jobname = RPC.execute("GetSignInName", job)
  local string = "Checked in as a " .. jobname
  TriggerEvent("np-phone:SendNotify", string, "jobcenter", "Job Center")
end)

RegisterNetEvent("getPaycheck")
AddEventHandler("getPaycheck", function(job)
  if not signedin then exports['np-phone']:SendAlert('error', 'You are not signed in', 3000) return end
  if signedinjob == nil then exports['np-phone']:SendAlert('error', 'You are not signed in', 3000) return end
  if tostring(signedinjob) ~= tostring(job) then exports['np-phone']:SendAlert('error', 'You are not signed into this job', 3000) return end
...
AddEventHandler("OfferCompleted", function(groupId, members)
    ...
    for k,v in pairs(members) do
        if tonumber(v.src) == tonumber(src) then
            --Wait(400)
            TriggerEvent("np-phone:SendNotify", "The offer was completed successfully.", "jobcenter", "Job Offer") 
        end
    end
end)
...
AddEventHandler("OfferNotCompleted", function(groupId, members)
    ...
    for k,v in pairs(members) do
        if tonumber(v.src) == tonumber(src) then
            Wait(400)
            TriggerEvent("np-phone:SendNotify", "The offer was not completed.", "jobcenter", "Job Offer") 
        end
    end
end)

RegisterNetEvent("np-phone:JobNotify")
AddEventHandler("np-phone:JobNotify", function(title, text, bool, groupId)
...
RegisterNetEvent("np-phone:ClearJobNotify")
AddEventHandler("np-phone:ClearJobNotify", function(groupId, members)
    local clientId = PlayerId()
    local src = GetPlayerServerId(clientId)
    for k,v in pairs(members) do
        if tonumber(v.src) == tonumber(src) then
    ...
    for k,v in pairs(members) do
        if tonumber(v.src) == tonumber(src) then
            if tonumber(v.src) ~= tonumber(groupData.leader) then
                TriggerEvent("np-phone:JobNotify", task.header, task.task, false, groupId)
            else
                TriggerEvent("np-phone:JobNotify", task.header, task.task, true, groupId)
            end
        end
    end
end)
...
AddEventHandler("findFishingSpot", function(groupId, groupData, members, task)
    ...
    for k,v in pairs(members) do
        if tonumber(v.src) == tonumber(src) then
            TriggerEvent("np-fishing:jobEvent", true)
            nearFish = true
            nearFishing()
            if tonumber(v.src) ~= tonumber(groupData.leader) then
                TriggerEvent("np-phone:JobNotify", task.header, task.task, false, groupId)
...
AddEventHandler("letThemKnowSpotIsGood", function(groupId, members)
    ...
    for k,v in pairs(members) do
        if tonumber(v.src) == tonumber(src) then
            TriggerEvent("np-fishing:jobEvent", nil)
...
RegisterNUICallback("setJobsGps", function(data, cb)
    ...
    local y = tonumber(y)
    SetNewWaypoint(x, y)
    exports['np-phone']:SendAlert('inform', 'Updated GPS', 3000)
end)
...
RegisterNUICallback("joinGroup", function(data, cb)
    if not cooldown then
        ...
        TriggerEvent("np-phone:SendNotify", "You already have an active request.", "jobcenter", "Job Center")
    end
end)
...
Citizen.CreateThread(function()
    exports["np-polyzone"]:AddBoxZone("trucker_foreman", vector3(919.9, -1256.54, 25.53), 1.0, 1.8, {
        heading=30,
        minZ=22.13,
        maxZ=26.73
    })
    exports["np-polyzone"]:AddBoxZone("garbage_foreman", vector3(-353.94, -1545.68, 27.72), 3.0, 2.0, {
        heading=0,
        minZ=24.92,
        maxZ=28.92
    })
    exports["np-polyzone"]:AddBoxZone("waterandpower_foreman", vector3(442.99, -1969.18, 24.4), 1.4, 2.0, {
        heading=315,
        minZ=22.0,
        maxZ=26.0
    })
    exports["np-polyzone"]:AddBoxZone("postop_foreman", vector3(-417.43, -2792.96, 6.0), 3.2, 1.4, {
        heading=320,
        minZ=3.2,
        maxZ=7.2
      })
  ...
  Citizen.CreateThread(function()
      while listening do
          if IsControlJustReleased(0, 38) then
              ...
              local vehicle = RPC.execute("voidrp:garage:vehicleSpawn", 2053223216, { x = 912.4641723632813, y = -1258.9508056640626, z = 25.57445907592773, h = 43.63048553466797 }, license_plate)
              ...
              local vehicle = RPC.execute("voidrp:garage:vehicleSpawn", -1255698084, { x = -332.53625488281, y = -1565.9274902344, z = 25.231986999512, h = 235.33598327637 }, license_plate)
              ...
              elseif job == "waterandpower" then
                ...
                local vehicle = RPC.execute("voidrp:garage:vehicleSpawn", hash, { x = 446.18966, y = -1963.951, z = 22.943767, h = 219.07287 }, license_plate)
            ...
            elseif job == "postop" then
                ...
                local vehicle = RPC.execute("voidrp:garage:vehicleSpawn", hash, { x = -403.8789, y = -2787.399, z = 5.902821, h = 316.56219 }, license_plate)
...
function enterPoly(name, data, job)
    ...
    exports["np-interface"]:showInteraction("[E] Ask the foreman for a vehicle")
end
...
RegisterNetEvent('np-jobs:sanitationWorker:pickupTrash')
AddEventHandler('np-jobs:sanitationWorker:pickupTrash', function(pArgs, pEntity, pContext)
    ...
    if not found then
        if pickedUpTrash[pEntity] then exports['np-phone']:SendAlert('error', 'Empty...', 3000) return end
        ...
        if pickedUpTrash[pEntity] and tonumber(pickedUpTrash[pEntity]) > 2 then exports['np-phone']:SendAlert('error', 'Empty...', 3000) return end
...
RegisterNetEvent('np-jobs:sanitationWorker:vehicleTrash')
AddEventHandler('np-jobs:sanitationWorker:vehicleTrash', function(pArgs, pEntity, pContext)
    if signedinjob ~= "recycle" then return end
