...
RegisterNUICallback("np-ui:getCars", function(data, cb)
  local data = RPC.execute("voidrp:vehicles:getPlayerVehiclesWithCoordinates")
  ...
  for _, car in pairs(data) do
    if car.location then
      ...
      print('ERROR GETTING THE LOCATION OF THE VEHICLE', car.current_garage, car.license_plate, car.location)
    end
...
RegisterNUICallback("np-ui:carActionTrack", function(data, cb)
...
RegisterNUICallback("np-ui:carActionSpawn", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = '' }})
  ...
  local success, message = RPC.execute('voidrp:vehicles:respawnVehicle', vehicle_plate)
  ...
  if success then
    ...
    if message then
      ...
      --exports["np-vehicles"]:SetVehicleAppearance(veh, vehicle_plate.appearance)
      --exports["np-vehicles"]:SetVehicleMods(veh, vehicle_plate.mods)
