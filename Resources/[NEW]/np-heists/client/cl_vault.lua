    ...
    if pd >= 4 then
        if distance < 1.5 then
            if exports["np-inventory"]:hasEnoughOfItem("heistlaptop4", 1) then
                ...
                TriggerEvent('np-dispatch:bankrobbery')
                TriggerEvent("client:newStress",true, 150)
                exports['np-hacking']:OpenHackingGame(15, 6, 1, function(Success)
                    OnHackDone(Success)
                    DeleteObject(bag)
                    DeleteObject(laptop)
...
function StartLowerVaultKid()
    ...
    ClearPedTasksImmediately(ply)
    Wait(0)
    TaskGoStraightToCoord(ply, 272.02313, 231.2503, 97.683662-0.9, 2.0, -1, 0.0)  
    loadDicts()
    Wait(0)
    while GetIsTaskActive(ply, 35) do
        Wait(0)
    ...
    Wait(0)
    TaskPlayAnimAdvanced(ply, "anim@heists@ornate_bank@hack", "hack_enter", 272.02313, 231.2503, 97.683662 -0.9, 0, 0, 0, 1.0, 0.0, 8300, 0, 0.3, false, false, false)
    Wait(0)
    SetEntityHeading(ply, 341.11935)
    while IsEntityPlayingAnim(ply, "anim@heists@ornate_bank@hack", "hack_enter", 3) do
        Wait(0)
...
RegisterNetEvent('nicx:createCrates')
AddEventHandler('nicx:createCrates', function(toggle)
    if toggle == true then
...
function CreatenBrakeCrates()
    ...
    TriggerEvent("nicx:createCrates" ,true)
