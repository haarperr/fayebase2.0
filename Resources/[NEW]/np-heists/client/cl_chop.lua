...
Citizen.CreateThread(function()
    exports["np-polyzone"]:AddBoxZone("chop_zone", vector3(1359.1, -2097.81, 52.0), 15, 15, {
    heading=40,
    --debugPoly=true,
    minZ=51.0,
    maxZ=55.0
...
AddEventHandler("np-polyzone:enter", function(zone)
    if zone == "chop_zone" and GetVehiclePedIsIn(PlayerPedId(), false) == voidChopCar then
        ...
        voidStartChopping()
        exports['np-interface']:showInteraction('[E] Start Chopping')
    end
end)

function voidStartChopping()
	Citizen.CreateThread(function()
...
function ChopVehicleTyre(vehicle, boneID, tyreIndex)
    ...
    local finished = exports["np-taskbar"]:taskBar(10000,"Slicing Vehicle Tyre")
...
function ChopVehicleDoor(vehicle, boneID, doorIndex)
    ...
    FreezeEntityPosition(GetPlayerPed(-1),true)
    local finished = exports["np-taskbar"]:taskBar(14000,"Messing With Door")
...
function ChopVehicleRemains(vehicle, boneID)
    ...
    FreezeEntityPosition(GetPlayerPed(-1),true)
    local finished = exports["np-taskbar"]:taskBar(25000,"Seeing Whats Left ..")
    ...
    TriggerEvent('void-chop:cooldown')
    return success
end

RegisterNetEvent('void-chop:cooldown')
AddEventHandler('void-chop:cooldown', function()
    CoolDownActive = true
    Citizen.Wait(900000)
...
function ChopVehiclePart(vehicle)
    ...
    if success then
        TriggerServerEvent("np-chopshop:vehicle_loot", bone.type)
...
RegisterNetEvent('np-chopshop:clock-in')
AddEventHandler('np-chopshop:clock-in', function()
    if CoolDownActive then
...
RegisterNetEvent('np-chopshop:clock-out')
AddEventHandler('np-chopshop:clock-out', function()
    if ClockedIntoChop then
...
function voidCreateChopCar()
	if DoesEntityExist(voidChopCar) then
	    SetVehicleHasBeenOwnedByPlayer(voidChopCar,false)
		SetEntityAsNoLongerNeeded(voidChopCar)
		DeleteEntity(voidChopCar)
	end

    local voidChopVehicle = GetHashKey(ChopCars[math.random(#ChopCars)])
    RequestModel(voidChopVehicle)
    while not HasModelLoaded(voidChopVehicle) do
        Citizen.Wait(0)
    end
    ...
    voidChopCar = CreateVehicle(voidChopVehicle, x, y, z, h, true, false)

	local pos = GetEntityCoords(voidChopCar, false)
    Citizen.CreateThread(function()
        while true do
          Citizen.Wait(5)
          if GetVehiclePedIsIn(PlayerPedId(), false) == voidChopCar then
