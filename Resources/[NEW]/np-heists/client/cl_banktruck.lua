...
RegisterNetEvent('np-heists:start_loop')
AddEventHandler('np-heists:start_loop', function()
    pickup = true
    TriggerEvent("np-heists:pick_cash")
    Wait(180000)
    Wait(additionalWait)
    pickup = false
end)

RegisterNetEvent('np-heists:pick_cash')
AddEventHandler('np-heists:pick_cash', function()
        ...
        if aDist < 2.0 and not LootedTruck then
            ...
            if IsDisabledControlJustReleased(0, 38) then
                if not LootedTruck then
                    ...
                    local BankTruckBarLoot = exports['np-taskbar']:taskBar(60000, 'Looting the truck')
                    if BankTruckBarLoot == 100 then
                        FreezeEntityPosition(PlayerPedId(), false)
                        TriggerEvent('np-heists:banktruck_loot') 
...
RegisterNetEvent('np-heists:banktruck_loot')
AddEventHandler('np-heists:banktruck_loot', function()
...
RegisterNetEvent('np-heists:spawn_peds')
AddEventHandler('np-heists:spawn_peds', function(veh)
...
RegisterNetEvent('np-heists:banktruck:cl')
AddEventHandler('np-heists:banktruck:cl', function()
