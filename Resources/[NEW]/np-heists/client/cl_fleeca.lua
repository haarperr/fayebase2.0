...
RegisterInterfaceCallback('np-fleeca:email_1', function(data, cb)
...
RegisterInterfaceCallback('np-fleeca:email_2', function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
...
RegisterInterfaceCallback('np-fleeca:email_3', function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
...
RegisterInterfaceCallback('np-fleeca:email_4', function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
...
RegisterInterfaceCallback('np-fleeca:email_5', function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
...
RegisterInterfaceCallback('np-fleeca:email_6', function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
...
RegisterNetEvent("aspect:bankemail", function(bankname)
    local username = "FLEECA"..(math.random (1,9))
    local password = math.random (10,99)
...
RegisterNetEvent('np-robbery:usb')
AddEventHandler('np-robbery:usb', function()
    local ped = GetPlayerPed(-1)
    local pos = GetEntityCoords(ped)
            ...
            if closestBank ~= nil then
                ...
                if dist < 1.5 then				
                    if not Config.SmallBanks[closestBank]["isOpened"] then 
                        -- if Police >= 4 then
                            TriggerServerEvent("np-heists:availabilty")
                            TriggerEvent('np-dispatch:bankrobbery')
                            TriggerServerEvent('aspect:retrievelogindata', closestBank)
                            TriggerEvent("client:newStress",true,5000)
                            FreezeEntityPosition(GetPlayerPed(-1), false)
                            Citizen.Wait(5000)
                            TriggerEvent('aspect:starthacking', closestBank)
                        -- else
                        --     TriggerEvent("DoLongHudText", "You're missing equiptment...", 2)
                        -- end
                    else
...
RegisterNetEvent('np-robbery:client:setBankState')
AddEventHandler('np-robbery:client:setBankState', function(bankId, state)
...
RegisterNetEvent('np-robbery:client:enableAllBankSecurity')
AddEventHandler('np-robbery:client:enableAllBankSecurity', function()
    for k, v in pairs(Config.SmallBanks) do
        Config.SmallBanks[k]["alarm"] = true
    end
end)

RegisterNetEvent('np-robbery:client:disableAllBankSecurity')
AddEventHandler('np-robbery:client:disableAllBankSecurity', function()
    for k, v in pairs(Config.SmallBanks) do
        Config.SmallBanks[k]["alarm"] = false
    end
end)

RegisterNetEvent('np-robbery:client:BankSecurity')
AddEventHandler('np-robbery:client:BankSecurity', function(key, status)
...
RegisterNetEvent('np-robbery:client:setLockerState')
AddEventHandler('np-robbery:client:setLockerState', function(bankId, lockerId, state, bool)
    if bankId == "pacific" then	
        print('[QUEER]')
    else
...
RegisterNetEvent('np-robbery:client:ResetFleecaLockers')
AddEventHandler('np-robbery:client:ResetFleecaLockers', function(BankId)
...
RegisterNetEvent("fleeca:success:panel", function()
    TriggerEvent("addEmail", {title = "Anonymous", subject = "Bank Security Status", message = "Wait 5 Minutes for the door to open"})
    Citizen.Wait(300000)
     TriggerEvent("addEmail", {title = "Anonymous", subject = "Bank Security Status", message = "Door open! Check out the back and loot the trolleys."})
     TriggerEvent('np-dispatch:bankrobbery')
     TriggerServerEvent('np-robbery:server:setBankState', closestBank, true)

     if closestBank == 1 then
        print('[np-HEISTS] CREATE FLEECA PINK CAGE TROLLEYS')
        TriggerEvent('voidrp:create:props:trolleys:pink:cage:bank')
     elseif closestBank == 2 then
        print('[np-HEISTS] CREATE FLEECA LEGION SQUARE TROLLEYS')
        TriggerEvent('voidrp:create:props:trolleys:legion:sqaure:bank')
     elseif closestBank == 3 then
        print('[np-HEISTS] CREATE FLEECA HAWICK AVE TROLLEYS')
        TriggerEvent('voidrp:create:props:trolleys:hawick:ave:bank')
     elseif closestBank == 4 then
        print('[np-HEISTS] CREATE FLEECA DEL PERRO TROLLEYS')
        TriggerEvent('voidrp:create:props:trolleys:del:perrro:blvd')
     elseif closestBank == 5 then
        print('[np-HEISTS] CREATE FLEECA GREAT OCEAN TROLLEYS')
        TriggerEvent('voidrp:create:props:trolleys:great:ocean:highway')
     elseif closestBank == 6 then
        print('[np-HEISTS] CREATE FLEECA ROUTE 68 TROLLEYS')
        TriggerEvent('voidrp:create:props:trolleys:route:68:bank')
     end
     DeleteObject(laptop)
 end)
...
AddEventHandler("fleeca:loot", function()
    local card = exports["np-taskbar"]:taskBar(40000,"Taking cash...")
    if card == 100 then
...
RegisterNetEvent('voidrp:create:props:trolleys:legion:sqaure:bank')
AddEventHandler('voidrp:create:props:trolleys:legion:sqaure:bank', function()
...
RegisterNetEvent('voidrp:create:props:trolleys:del:perrro:blvd')
AddEventHandler('voidrp:create:props:trolleys:del:perrro:blvd', function()
...
RegisterNetEvent('voidrp:create:props:trolleys:great:ocean:highway')
AddEventHandler('voidrp:create:props:trolleys:great:ocean:highway', function()
...
RegisterNetEvent('voidrp:create:props:trolleys:pink:cage:bank')
AddEventHandler('voidrp:create:props:trolleys:pink:cage:bank', function()
...
RegisterNetEvent('voidrp:create:props:trolleys:hawick:ave:bank')
AddEventHandler('voidrp:create:props:trolleys:hawick:ave:bank', function()
...
RegisterNetEvent('voidrp:create:props:trolleys:route:68:bank')
AddEventHandler('voidrp:create:props:trolleys:route:68:bank', function()
...
AddEventHandler('np-heists:fleeca_available', function()
	...
	local FleecaAvail = {
		...
		{
            title = "Purchase Laptop & Information",
            description = "$20,000 And a ~#@*%(#@",
            action = "np-illegalactivities:buylaptop1",
        },
        {
            title = "Get Bank Information",
            children = {
                {
                    title = "$10,000  Per Bank Information",
                },
                {
                    title = "Bank 1",
                    description = "Legion Square",
                    action = "np-fleeca:email_1",
                },
                {
                    title = "Bank 2",
                    description = "Boulevard Del Perro",
                    action = "np-fleeca:email_2",
                },
                {
                    title = "Bank 3",
                    description = "Great Ocean Highway",
                    action = "np-fleeca:email_3",
                },
                {
                    title = "Bank 4",
                    description = "Pink Cage",
                    action = "np-fleeca:email_4",
                },
                {
                    title = "Bank 5",
                    description = "Hawick Avenue",
                    action = "np-fleeca:email_5",
                },
                {
                    title = "Bank 6",
                    description = "Route 68",
                    action = "np-fleeca:email_6",
                },
            }
        },
    }
    exports["np-interface"]:showContextMenu(FleecaAvail)
end)
...
AddEventHandler('np-heists:fleeca_unavailable', function()
    ...
    exports["np-interface"]:showContextMenu(FleecaUnavail)
end)