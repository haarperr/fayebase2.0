...
RegisterNUICallback("np-racing:pingReject", function(data, cb)
  TriggerServerEvent("np-ping:rejected", lastPingerId)
  lastPingerId = -1
  pingCoords = vector3(0, 0, 0)
end)

function hasDecrypt()
    if exports["np-inventory"]:hasEnoughOfItem("racingusb",1,false) then
      return true
    else
      return false
    end
...
function hasTablet()
    if exports['np-inventory']:hasEnoughOfItem("racingipad", 1) and exports['np-inventory']:hasEnoughOfItem("racingusb", 1) then
      return true
    else
      return false
    end
...
RegisterNetEvent("np-racing:openGUI")
AddEventHandler("np-racing:openGUI", function()
  print('[np-RACING] OPENING GUI...')
  print('[np-RACING] ANIMATION LOADED...')
  openGui()
  print('[np-RACING] GUI OPENED...')
end)
...
function closeGui()
  print('[np-RACING] CLOSING GUI...')
  SetNuiFocus(false,false)
  SendNUIMessage({openPhone = false})
  StopAnimTask(PlayerPedId(), "amb@code_human_in_bus_passenger_idles@female@tablet@base", "base", 1.0)
  TriggerEvent("destroyPropPhone")
  print('[np-RACING] STOPPING ANIMATION...')
  guiEnabled = false
  pTabletOpen = false
  recentopen = true
  Citizen.Wait(500)
  recentopen = false
  print('[np-RACING] GUI CLOSED...')
end

RegisterNetEvent('np-racing:UpdateStatePhone')
AddEventHandler('np-racing:UpdateStatePhone', function()
    Wait(5)
