...
$(document).ready(function () {
    ...
    $.post('http://np-racing/getWeather', JSON.stringify({}));
    ...
    document.onkeyup = function (data) {
        ...
        if (data.which == 27) {
            $.post('http://np-racing/close', JSON.stringify({}));
        }
    }
        ...
        if (action !== undefined) {
            switch (action) {
                case "yellow-pages-delete":
                    $.post('http://np-racing/deleteYP', JSON.stringify({}));
                    break;
                case "racing-create":
                ...
                case "newPostSubmit":
                    e.preventDefault();
                    $.post('http://np-racing/newPostSubmit', JSON.stringify({
                    ...
                    break;
                case "group-manage":
                    $.post('http://np-racing/manageGroup', JSON.stringify({ GroupID: $(this).data('action-data') }));
                    break;
                case "btnTaskGang":
                    manageGroup = $(this).data('action-data');
                    $.post('http://np-racing/btnTaskGang', JSON.stringify({}));
                    break;
                ...
                case "getCallHistory":
                    if (callStates[currentCallState] === "isCallInProgress" && currentContainer !== "incoming-call")
                        ...
                        $.post('http://np-racing/' + action, JSON.stringify({}));
                    break;
                case "spotify":
    ...
    window.addEventListener('message', function (event) {
        ...
        $("#housing-found-form").submit(function (event) {
            ...
            $.post('http://np-racing/housing:buyProperty', JSON.stringify({}));
        });        
            ...
            case "racing:hud:update":
                switch (item.hudState) {
                    ...
                    case "finished":
                        ...
                        drawRaceStats();
                        $.post('http://np-racing/race:completed', JSON.stringify({
...
function openBrowser(url) {
    ...
    $.post('http://np-racing/btnCamera', JSON.stringify({}));
    $("#browser").fadeIn(300);
}
...
function addMessage(item) {
    ...
    element.click(function () {
        $.post('http://np-racing/messageRead', JSON.stringify({ sender: item.sender, receiver: item.receiver, displayName: item.msgDisplayName }));
    });
...
function addMessageOther(item) {
    ...
    element.id = item.id;
    element.click(function () {
        $.post('http://np-racing/messageRead', JSON.stringify({ sender: item.sender, receiver: item.receiver, displayName: receiver, clientPhone: item.clientNumber }));
    });
    $(".messages-entries").prepend(element);
}
...
function updateSettings()
{
    switch (currentSettingWindow) {
        case "tokovoip":
            ...
            break;
        case "control":
            $.post('http://np-racing/settingsUpdateToko', JSON.stringify({tag: "controlUpdate", controls: currentBinds}));
            break;
        case "browser":
            break;
        default:
...
function ResetSettings()
{
    switch (currentSettingWindow) {
        case "tokovoip":
            $.post('http://np-racing/settingsResetToko', JSON.stringify());
            break;
        case "control":
            $.post('http://np-racing/settingsResetControls', JSON.stringify());
            break;
        case "browser":
            break;
        default:
...
async function updateTokoSettings()
{
    ...
    await delayedLog();
    $.post('http://np-racing/settingsUpdateToko', JSON.stringify({
        tag: "settings",
...
$('#racing-create-form').on('reset', function (e) {
    $.post('http://np-racing/racing:map:cancel', JSON.stringify({}));
});

$('#racing-start-tracks').on('change', function (e) {
    let selectedMap = $(this).val();
    if(maps[selectedMap] !== undefined) {
        $.post('http://np-racing/racing:map:removeBlips', JSON.stringify({}));
        $.post('http://np-racing/racing:map:load', JSON.stringify({ id: selectedMap}));
...
$('#racing-start').submit(function (e) {
    ...
    $.post('http://np-racing/racing:event:start', JSON.stringify({
...
$('#racing-create-form').submit(function (e) {
    e.preventDefault();
    $.post('http://np-racing/racing:map:save', JSON.stringify({
...
$("#real-estate-sell-form").submit(function (e) {
    e.preventDefault();
    $.post('http://np-racing/btnAttemptHouseSale', JSON.stringify({
...
$('#real-estate-transfer-form').submit(function (e) {
    e.preventDefault();
    $.post('http://np-racing/btnTransferHouse', JSON.stringify({
...
$("#group-manage-pay-form").submit(function (e) {
    ...
    $.post('http://np-racing/payGroup', JSON.stringify({
...
$("#group-manage-rank-form").submit(function (e) {
    e.preventDefault();
    $.post('http://np-racing/promoteGroup', JSON.stringify({
        gangid: escapeHtml($(".group-manage-company-name").data('group-id')),
...
$("#group-manage-bank-form").submit(function (e) {
    ...
    $.post('http://np-racing/bankGroup', JSON.stringify({
        gangid: escapeHtml($(".group-manage-company-name").data('group-id')),
...
$("#group-tasks-assign-modal-form").submit(function (e) {
    ...
    $.post('http://np-racing/btnGiveTaskToPlayer', JSON.stringify({
...
$("#contacts-form").submit(function (e) {
    ...
    $.post('http://np-racing/newContactSubmit', JSON.stringify({
        name: clean,
...
$("#wallpaper-form").submit(function (e) {
    ...
    $.post('http://np-racing/updateMyWallpaper', JSON.stringify({
        name: escapedName,
    }));
...
$("#stock-form").submit(function (event) {
    event.preventDefault();
    $.post('http://np-racing/stocksTradeToPlayer', JSON.stringify({
...
$("#twat-form").submit(function (event) {
    event.preventDefault();
    $.post('http://np-racing/newTwatSubmit', JSON.stringify({
...
$("#call-form").submit(function (event) {
    event.preventDefault();
    $.post('http://np-racing/callContact', JSON.stringify({
        name: '',
...
$("#new-message-form").submit(function (event) {
    ...
    $.post('http://np-racing/newMessageSubmit', JSON.stringify({
    ...
    switch (currentContainer) {
        case "message":
            setTimeout(function () {
                ...
                $.post('http://np-racing/messageRead', JSON.stringify({ sender: sender, receiver: receiver, displayName: displayName }));
            }, 300);
            break;
        case "messages":
            setTimeout(function () {
                $.post('http://np-racing/messages', JSON.stringify({}));
            }, 300);
            break;
    }
...
$('.racing-map-delete-confirm').click(function () {  
    ...
    $.post('http://np-racing/racing:map:delete', JSON.stringify({ id: raceMap }));
    $('.racing-delete-confirm').fadeOut(150)
...
$('#real-estate-evict-modal-accept').click(function () {
    $.post('http://np-racing/btnEvictHouse', JSON.stringify({}));
...
$('.twatter-toggle-notification').click(function () {
    ...
    $.post('http://np-racing/btnNotifyToggle', JSON.stringify({}));
});



$('.account-information-toggle-pager').click(function () {
    $.post('http://np-racing/btnPagerToggle', JSON.stringify({}));
...
$('.racing-entries').on('click', '.racing-entries-join', function () {
    $.post('http://np-racing/racing:event:join', JSON.stringify({ identifier: $(this).data('id') }));
});

$('.keys-entries').on('click', '.manage-keys', function () {
    $.post('http://np-racing/retrieveHouseKeys', JSON.stringify({}));
});

$('.keys-entries').on('click', '.remove-shared-key', function(e) {
    $.post('http://np-racing/removeSharedKey', JSON.stringify({
...
$('.manage-keys-entries').on('click', '.manage-keys-remove', function () {
    $.post('http://np-racing/removeHouseKey', JSON.stringify({
...
$('.group-tasks-entries').on('click', '.group-tasks-track', function () {
    $.post('http://np-racing/trackTaskLocation', JSON.stringify({ TaskIdentifier: $(this).data('id') }));
});

$('.delivery-job-entries').on('click', '.delivery-job-accept', function (e) {
    $.post('http://np-racing/selectedJob', JSON.stringify({ jobType: $(this).data('job-type'), jobId: $(this).data('job-id') }));
});
...
$('.garage-container-entries-wrapper').on('click', '.garage-spawn', function (e) {
    e.preventDefault();
    $.post('http://np-racing/vehspawn', JSON.stringify({ vehplate: $(this).data('plate') }));
    $.post('http://np-racing/btnGarage', JSON.stringify({}));
});

$('.garage-container-entries-wrapper').on('click', '.garage-track', function () {
    $.post('http://np-racing/vehtrack', JSON.stringify({ vehplate: $(this).data('plate') }));
});

$('.garage-entries').on('click', '.garage-pay', function (e) {
    $.post('http://np-racing/vehiclePay', JSON.stringify({ vehiclePlate: $(this).data('plate') }));
    setTimeout(function () {
        $.post('http://np-racing/btnGarage', JSON.stringify({}));
    }, 1500);
});
...
$('.gps-entries, .keys-entries').on('click', '.gps-location-click', function () {
    $.post('http://np-racing/loadUserGPS', JSON.stringify({ house_id: $(this).data('house-id'), house_type: $(this).data('house-type') }));
})

$('.contacts-entries-wrapper, .call-history-entries-wrapper').on('click', '.btn-contacts-call', function () {
    $.post('http://np-racing/callContact', JSON.stringify({ name: $(this).data('name'), number: $(this).data('number') }));
});
...
$('#confirm-modal-accept').click(function (event) {
    $.post('http://np-racing/removeContact', JSON.stringify({ name: $(this).data('name'), number: $(this).data('number') }));
...
$("#twat-gifword").change(function() {
   ...
   $(document).on('click', '#twat-sendimg', function (e) {
    ...
    $.post('http://np-racing/phone:selfie', JSON.stringify({}));
    e.preventDefault();
    $.post('http://np-racing/PostNewImage', JSON.stringify({}),
        function (url) {
...
$(document).ready( function() {
    $("#pinger_button").click(function() {
        let id = $("#pingerid").val()
        $.post('https://np-racing/pingSend', JSON.stringify({
            number: id,
        }));
    })
})
...
function SetWaypointPostOP() {
    $.post('http://np-racing/SetPostOpNewWaypoint', JSON.stringify({}));
}

function SetWaypointFarming() {
    $.post('http://np-racing/SetFarmerJobWaypoint', JSON.stringify({}));
}

function SetWaypointFarmingSales() {
    $.post('http://np-racing/SetFarmerJobWaypointSale', JSON.stringify({}));
}

function SetWaypointFarmingProcess() {
    $.post('http://np-racing/SetFarmerJobWaypointProcess', JSON.stringify({}));
}

function SetWaypointMining() {
    $.post('http://np-racing/SetMiningNewWaypoint', JSON.stringify({}));
}

function SetWaypointWaterNPower() {
    $.post('http://np-racing/SetWaterNPowerNewWaypoint', JSON.stringify({}));
}

function SetWayPointGarbage() {
    $.post('http://np-racing/SetGarbageNewWaypoint', JSON.stringify({}));
}

function SetWayPointChickenJob() {
    $.post('http://np-racing/SetChickenNewWaypoint', JSON.stringify({}));
}

function SetWayPointFishingJob() {
    $.post('http://np-racing/SetFishingNewWaypoint', JSON.stringify({}));
}

function SetWayPointHunting() {
    $.post('http://np-racing/SetHuntingNewWaypoint', JSON.stringify({}));
}

// Sale Locations

function MiningSales() {
    $.post('http://np-racing/SetMiningSalesLocation', JSON.stringify({}));
}

function ChickenSales() {
    $.post('http://np-racing/SetChickenSalesLocation', JSON.stringify({}));
}

function FishingSales() {
    $.post('http://np-racing/SetFishingSalesLocation', JSON.stringify({}));
}

function HuntingSales() {
    $.post('http://np-racing/SetHuntingSalesLocation', JSON.stringify({}));
}
