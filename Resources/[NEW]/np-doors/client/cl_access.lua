...
function hasSecuredAccess(pId, pType)
    ...
    local mypasses = RPC.execute("np-business:business:GetEmploymentInformation")
    for i=1, #mypasses do
    if secured.access.business and secured.access.business[mypasses[i]["businessid"]] == true then
    local hasPermission = RPC.execute("np-business:business:hasPermission", mypasses[i]["businessid"], "key_access")
    if hasPermission then
        if secured.access.permission then
            local hasPermission = RPC.execute("np-business:business:hasPermission", mypasses[i]["businessid"], secured.access.permission[1])
            if hasPermission then
            accessCheckCache[pType][pId] = true
                return true
            end
