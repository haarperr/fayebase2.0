	...
	while tasking do
		...
		if dstcheck < 2.0 and pedCreated then
			...
			if IsControlJustReleased(0,38) then
				...
				local finished = exports["np-taskbar"]:taskBar(2000,"Making An Exchange")
...
Citizen.CreateThread(function()
	exports["np-polyzone"]:AddBoxZone("rolex_run_start", vector3(182.79, -1319.47, 29.32), 2, 2, {
...
AddEventHandler('np-polyzone:enter', function(name)
    if name == "rolex_run_start" then
		...
		if not RolexRun then
			exports["np-interface"]:showInteraction("[E] Start Run ($500)")
		else 
			exports["np-interface"]:showInteraction("Finish Your Run")
		end
    end
end)
...
function RolexLocation()
	Citizen.CreateThread(function()
        while RolexSpot do
            Citizen.Wait(5)
			if IsControlJustReleased(0, 38) then
				if exports['np-inventory']:hasEnoughOfItem('rolexwatch', 1) or exports['np-inventory']:hasEnoughOfItem('stolenpsp', 1) or exports['np-inventory']:hasEnoughOfItem('stolens8', 1) or exports['np-inventory']:hasEnoughOfItem('stolenBrokenGoods', 1) or exports['np-inventory']:hasEnoughOfItem('stolen2ctchain', 1) or exports['np-inventory']:hasEnoughOfItem('stolenraybans', 1) or exports['np-inventory']:hasEnoughOfItem('stolencasiowatch', 1) then
					if not RolexRun then
