...
function SyncWheelFitment()
    if isPlyWhitelisted then
        ...
        if didPlyAdjustFitments then
            ...
            RPC.execute("np-wheelfitment_sv:saveWheelfitment", plate ,currentFitmentsToSet)
...
Citizen.CreateThread(function()
    ...
    exports["np-polyzone"]:AddBoxZone("np-wheelfitment:zone1", vector3(136.0, -3030.36, 6.52), 7.8, 4.2, {
        name="np-wheelfitment:zone1",
        heading=1,
        --debugPoly=true
      })
end)
...
RegisterNetEvent('np-fitment:devaleks',function()
    devwheelMenu ()
end)
...
AddEventHandler("np-wheelfitment_cl:applySavedWheelFitment", function(plyVeh, VehicleWheel)
